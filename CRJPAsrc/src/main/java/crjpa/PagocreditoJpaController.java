...
public class PagocreditoJpaController implements Serializable {
	...
	public void create(Pagocredito pagocredito) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (idCuentacredito != null) {
				idCuentacredito = em.getReference(idCuentacredito.getClass(), idCuentacredito.getId());
	...
	public void edit(Pagocredito pagocredito) throws NonexistentEntityException, Exception {
		...
		try {
			...
			Transaccion idTransaccionOld = persistentPagocredito.getIdTransaccion();
			Transaccion idTransaccionNew = pagocredito.getIdTransaccion();
			Cuentacredito idCuentacreditoOld = persistentPagocredito.getIdCuentacredito();
			...
			if (idCuentacreditoNew != null) {
				idCuentacreditoNew = em.getReference(idCuentacreditoNew.getClass(), idCuentacreditoNew.getId());
			...
			if (idCuentacreditoOld != null && !idCuentacreditoOld.equals(idCuentacreditoNew)) {
				idCuentacreditoOld.getPagocreditoList().remove(pagocredito);
				idCuentacreditoOld = em.merge(idCuentacreditoOld);
			}
			if (idCuentacreditoNew != null && !idCuentacreditoNew.equals(idCuentacreditoOld)) {
