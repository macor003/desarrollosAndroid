...
public class CuentacreditoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Cuentacredito cuentacredito) throws PreexistingEntityException, Exception {
		...
		try {
			...
			Cliente numeroIdentificacionCliente = cuentacredito.getNumeroIdentificacionCliente();
			if (numeroIdentificacionCliente != null) {
				...
				cuentacredito.setNumeroIdentificacionCliente(numeroIdentificacionCliente);
			}
			List<Pagocredito> attachedPagocreditoList = new ArrayList<Pagocredito>();
			for (Pagocredito pagocreditoListPagocreditoToAttach : cuentacredito.getPagocreditoList()) {
			...
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getCuentacreditoList().add(cuentacredito);
				numeroIdentificacionCliente = em.merge(numeroIdentificacionCliente);
			}
			for (Pagocredito pagocreditoListPagocredito : cuentacredito.getPagocreditoList()) {
				...
				if (oldIdCuentacreditoOfPagocreditoListPagocredito != null) {
					oldIdCuentacreditoOfPagocreditoListPagocredito.getPagocreditoList().remove(
			...
			if (findCuentacredito(cuentacredito.getId()) != null) {
				throw new PreexistingEntityException("Cuentacredito " + cuentacredito + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Cuentacredito cuentacredito) throws IllegalOrphanException, NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Cuentacredito persistentCuentacredito = em.find(Cuentacredito.class, cuentacredito.getId());
			Cliente numeroIdentificacionClienteOld = persistentCuentacredito.getNumeroIdentificacionCliente();
			Cliente numeroIdentificacionClienteNew = cuentacredito.getNumeroIdentificacionCliente();
			List<Pagocredito> pagocreditoListOld = persistentCuentacredito.getPagocreditoList();
			List<Pagocredito> pagocreditoListNew = cuentacredito.getPagocreditoList();
			...
			if (numeroIdentificacionClienteNew != null) {
				...
				cuentacredito.setNumeroIdentificacionCliente(numeroIdentificacionClienteNew);
			}
			...
			for (Pagocredito pagocreditoListNewPagocreditoToAttach : pagocreditoListNew) {
				pagocreditoListNewPagocreditoToAttach = em
						.getReference(pagocreditoListNewPagocreditoToAttach.getClass(),
			...
			if (numeroIdentificacionClienteOld != null
				...
				numeroIdentificacionClienteOld.getCuentacreditoList().remove(cuentacredito);
			...
			if (numeroIdentificacionClienteNew != null
				...
				numeroIdentificacionClienteNew.getCuentacreditoList().add(cuentacredito);
			...
			for (Pagocredito pagocreditoListNewPagocredito : pagocreditoListNew) {
				if (!pagocreditoListOld.contains(pagocreditoListNewPagocredito)) {
					Cuentacredito oldIdCuentacreditoOfPagocreditoListNewPagocredito = pagocreditoListNewPagocredito
							.getIdCuentacredito();
					pagocreditoListNewPagocredito.setIdCuentacredito(cuentacredito);
					...
					if (oldIdCuentacreditoOfPagocreditoListNewPagocredito != null
							&& !oldIdCuentacreditoOfPagocreditoListNewPagocredito.equals(cuentacredito)) {
						oldIdCuentacreditoOfPagocreditoListNewPagocredito.getPagocreditoList().remove(
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findCuentacredito(id) == null) {
					throw new NonexistentEntityException("The cuentacredito with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The cuentacredito with id " + id + " no longer exists.", enfe);
			}
			List<String> illegalOrphanMessages = null;
			List<Pagocredito> pagocreditoListOrphanCheck = cuentacredito.getPagocreditoList();
			for (Pagocredito pagocreditoListOrphanCheckPagocredito : pagocreditoListOrphanCheck) {
				if (illegalOrphanMessages == null) {
					illegalOrphanMessages = new ArrayList<String>();
				}
				illegalOrphanMessages.add("This Cuentacredito (" + cuentacredito
			...
			Cliente numeroIdentificacionCliente = cuentacredito.getNumeroIdentificacionCliente();
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getCuentacreditoList().remove(cuentacredito);
				numeroIdentificacionCliente = em.merge(numeroIdentificacionCliente);
			}
	 ...
	 * @return List<Cuentacredito>
	 */
	public List<Cuentacredito> findCuentacreditoEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Cuentacredito>
	 */
	private List<Cuentacredito> findCuentacreditoEntities(boolean all, int maxResults, int firstResult) {
