...
public class TransaccionarticuloJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Transaccionarticulo transaccionarticulo) throws PreexistingEntityException, Exception {
		if (transaccionarticulo.getTransaccionarticuloservicioList() == null) {
			transaccionarticulo.setTransaccionarticuloservicioList(new ArrayList<Transaccionarticuloservicio>());
		}
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Tipodescuento idTipodescuento = transaccionarticulo.getIdTipodescuento();
			...
			if (idUnidadventa != null) {
				idUnidadventa = em.getReference(idUnidadventa.getClass(), idUnidadventa.getId());
			...
			Tasaimpuestovalor idTasaimpuestovalor = transaccionarticulo.getIdTasaimpuestovalor();
			if (idTasaimpuestovalor != null) {
				idTasaimpuestovalor = em.getReference(idTasaimpuestovalor.getClass(), idTasaimpuestovalor.getId());
			...
			transaccionarticulo.setTransaccionarticuloservicioList(attachedTransaccionarticuloservicioList);
			em.persist(transaccionarticulo);
			if (idTipodescuento != null) {
				idTipodescuento.getTransaccionarticuloList().add(transaccionarticulo);
				idTipodescuento = em.merge(idTipodescuento);
			}
			if (idUnidadventa != null) {
				// idUnidadventa.getTransaccionarticuloList().add(transaccionarticulo);
				idUnidadventa = em.merge(idUnidadventa);
			}
			if (idArticulo != null) {
				idArticulo.getTransaccionarticuloList().add(transaccionarticulo);
				idArticulo = em.merge(idArticulo);
			}
			if (idTransaccion != null) {
				idTransaccion.getTransaccionarticuloList().add(transaccionarticulo);
				idTransaccion = em.merge(idTransaccion);
			}
			if (idTasaimpuestovalor != null) {
				idTasaimpuestovalor.getTransaccionarticuloList().add(transaccionarticulo);
				idTasaimpuestovalor = em.merge(idTasaimpuestovalor);
			}
			if (idPromocion != null) {
				idPromocion.getTransaccionarticuloList().add(transaccionarticulo);
			...
			if (findTransaccionarticulo(transaccionarticulo.getId()) != null) {
				throw new PreexistingEntityException("Transaccionarticulo " + transaccionarticulo + " already exists.",
						ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Transaccionarticulo transaccionarticulo) throws IllegalOrphanException,
			NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Transaccionarticulo persistentTransaccionarticulo = em.find(Transaccionarticulo.class,
					transaccionarticulo.getId());
			Tipodescuento idTipodescuentoOld = persistentTransaccionarticulo.getIdTipodescuento();
			Tipodescuento idTipodescuentoNew = transaccionarticulo.getIdTipodescuento();
			Unidadventa idUnidadventaOld = persistentTransaccionarticulo.getIdUnidadventa();
			Unidadventa idUnidadventaNew = transaccionarticulo.getIdUnidadventa();
			Articulo idArticuloOld = persistentTransaccionarticulo.getIdArticulo();
			Articulo idArticuloNew = transaccionarticulo.getIdArticulo();
			Transaccion idTransaccionOld = persistentTransaccionarticulo.getIdTransaccion();
			Transaccion idTransaccionNew = transaccionarticulo.getIdTransaccion();
			Tasaimpuestovalor idTasaimpuestovalorOld = persistentTransaccionarticulo.getIdTasaimpuestovalor();
			Tasaimpuestovalor idTasaimpuestovalorNew = transaccionarticulo.getIdTasaimpuestovalor();
			Promocion idPromocionOld = persistentTransaccionarticulo.getIdPromocion();
			...
			if (idUnidadventaNew != null) {
				idUnidadventaNew = em.getReference(idUnidadventaNew.getClass(), idUnidadventaNew.getId());
			...
			if (idTasaimpuestovalorNew != null) {
				idTasaimpuestovalorNew = em.getReference(idTasaimpuestovalorNew.getClass(),
						idTasaimpuestovalorNew.getId());
				transaccionarticulo.setIdTasaimpuestovalor(idTasaimpuestovalorNew);
			}
			...
			transaccionarticulo.setTransaccionarticuloservicioList(transaccionarticuloservicioListNew);
			transaccionarticulo = em.merge(transaccionarticulo);
			if (idTipodescuentoOld != null && !idTipodescuentoOld.equals(idTipodescuentoNew)) {
				idTipodescuentoOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idTipodescuentoOld = em.merge(idTipodescuentoOld);
			}
			if (idTipodescuentoNew != null && !idTipodescuentoNew.equals(idTipodescuentoOld)) {
				idTipodescuentoNew.getTransaccionarticuloList().add(transaccionarticulo);
				idTipodescuentoNew = em.merge(idTipodescuentoNew);
			}
			if (idUnidadventaOld != null && !idUnidadventaOld.equals(idUnidadventaNew)) {
				// idUnidadventaOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idUnidadventaOld = em.merge(idUnidadventaOld);
			}
			if (idUnidadventaNew != null && !idUnidadventaNew.equals(idUnidadventaOld)) {
				// idUnidadventaNew.getTransaccionarticuloList().add(transaccionarticulo);
				idUnidadventaNew = em.merge(idUnidadventaNew);
			}
			if (idArticuloOld != null && !idArticuloOld.equals(idArticuloNew)) {
				idArticuloOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idArticuloOld = em.merge(idArticuloOld);
			}
			if (idArticuloNew != null && !idArticuloNew.equals(idArticuloOld)) {
				idArticuloNew.getTransaccionarticuloList().add(transaccionarticulo);
				idArticuloNew = em.merge(idArticuloNew);
			}
			if (idTransaccionOld != null && !idTransaccionOld.equals(idTransaccionNew)) {
				idTransaccionOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idTransaccionOld = em.merge(idTransaccionOld);
			}
			if (idTransaccionNew != null && !idTransaccionNew.equals(idTransaccionOld)) {
				idTransaccionNew.getTransaccionarticuloList().add(transaccionarticulo);
				idTransaccionNew = em.merge(idTransaccionNew);
			}
			if (idTasaimpuestovalorOld != null && !idTasaimpuestovalorOld.equals(idTasaimpuestovalorNew)) {
				idTasaimpuestovalorOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idTasaimpuestovalorOld = em.merge(idTasaimpuestovalorOld);
			}
			if (idTasaimpuestovalorNew != null && !idTasaimpuestovalorNew.equals(idTasaimpuestovalorOld)) {
				idTasaimpuestovalorNew.getTransaccionarticuloList().add(transaccionarticulo);
				idTasaimpuestovalorNew = em.merge(idTasaimpuestovalorNew);
			}
			if (idPromocionOld != null && !idPromocionOld.equals(idPromocionNew)) {
				idPromocionOld.getTransaccionarticuloList().remove(transaccionarticulo);
				idPromocionOld = em.merge(idPromocionOld);
			}
			if (idPromocionNew != null && !idPromocionNew.equals(idPromocionOld)) {
				idPromocionNew.getTransaccionarticuloList().add(transaccionarticulo);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTransaccionarticulo(id) == null) {
					throw new NonexistentEntityException("The transaccionarticulo with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			try {
				transaccionarticulo = em.getReference(Transaccionarticulo.class, id);
				transaccionarticulo.getId();
			...
			Tipodescuento idTipodescuento = transaccionarticulo.getIdTipodescuento();
			if (idTipodescuento != null) {
				idTipodescuento.getTransaccionarticuloList().remove(transaccionarticulo);
				idTipodescuento = em.merge(idTipodescuento);
			}
			Unidadventa idUnidadventa = transaccionarticulo.getIdUnidadventa();
			if (idUnidadventa != null) {
				// idUnidadventa.getTransaccionarticuloList().remove(transaccionarticulo);
				idUnidadventa = em.merge(idUnidadventa);
			}
			Articulo idArticulo = transaccionarticulo.getIdArticulo();
			if (idArticulo != null) {
				idArticulo.getTransaccionarticuloList().remove(transaccionarticulo);
				idArticulo = em.merge(idArticulo);
			}
			Transaccion idTransaccion = transaccionarticulo.getIdTransaccion();
			if (idTransaccion != null) {
				idTransaccion.getTransaccionarticuloList().remove(transaccionarticulo);
				idTransaccion = em.merge(idTransaccion);
			}
			Tasaimpuestovalor idTasaimpuestovalor = transaccionarticulo.getIdTasaimpuestovalor();
			if (idTasaimpuestovalor != null) {
				idTasaimpuestovalor.getTransaccionarticuloList().remove(transaccionarticulo);
				idTasaimpuestovalor = em.merge(idTasaimpuestovalor);
			}
			Promocion idPromocion = transaccionarticulo.getIdPromocion();
			if (idPromocion != null) {
				idPromocion.getTransaccionarticuloList().remove(transaccionarticulo);
				idPromocion = em.merge(idPromocion);
			}
	 ...
	 * @return List<Transaccionarticulo>
	 */
	public List<Transaccionarticulo> findTransaccionarticuloEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Transaccionarticulo>
	 */
	private List<Transaccionarticulo> findTransaccionarticuloEntities(boolean all, int maxResults, int firstResult) {
