...
public class TransaccionclienteJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Transaccioncliente transaccioncliente) throws PreexistingEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Cliente numeroIdentificacionCliente = transaccioncliente.getNumeroIdentificacionCliente();
			if (numeroIdentificacionCliente != null) {
				...
				transaccioncliente.setNumeroIdentificacionCliente(numeroIdentificacionCliente);
			}
			Giroactividadeconomica idGiroactividadeconomica = transaccioncliente.getIdGiroactividadeconomica();
			if (idGiroactividadeconomica != null) {
				...
				transaccioncliente.setIdGiroactividadeconomica(idGiroactividadeconomica);
			}
			...
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getTransaccionclienteList().add(transaccioncliente);
				numeroIdentificacionCliente = em.merge(numeroIdentificacionCliente);
			}
			if (idGiroactividadeconomica != null) {
				idGiroactividadeconomica.getTransaccionclienteList().add(transaccioncliente);
				idGiroactividadeconomica = em.merge(idGiroactividadeconomica);
			}
			if (idTransaccion != null) {
				idTransaccion.getTransaccionclienteList().add(transaccioncliente);
				idTransaccion = em.merge(idTransaccion);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findTransaccioncliente(transaccioncliente.getId()) != null) {
				throw new PreexistingEntityException("Transaccioncliente " + transaccioncliente + " already exists.",
						ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Transaccioncliente transaccioncliente) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Transaccioncliente persistentTransaccioncliente = em.find(Transaccioncliente.class,
					transaccioncliente.getId());
			Cliente numeroIdentificacionClienteOld = persistentTransaccioncliente.getNumeroIdentificacionCliente();
			Cliente numeroIdentificacionClienteNew = transaccioncliente.getNumeroIdentificacionCliente();
			Giroactividadeconomica idGiroactividadeconomicaOld = persistentTransaccioncliente
					.getIdGiroactividadeconomica();
			Giroactividadeconomica idGiroactividadeconomicaNew = transaccioncliente.getIdGiroactividadeconomica();
			Transaccion idTransaccionOld = persistentTransaccioncliente.getIdTransaccion();
			Transaccion idTransaccionNew = transaccioncliente.getIdTransaccion();
			if (numeroIdentificacionClienteNew != null) {
				...
				transaccioncliente.setNumeroIdentificacionCliente(numeroIdentificacionClienteNew);
			}
			if (idGiroactividadeconomicaNew != null) {
				...
				transaccioncliente.setIdGiroactividadeconomica(idGiroactividadeconomicaNew);
			}
			...
			if (numeroIdentificacionClienteOld != null
				...
				numeroIdentificacionClienteOld.getTransaccionclienteList().remove(transaccioncliente);
			...
			if (numeroIdentificacionClienteNew != null
				...
				numeroIdentificacionClienteNew.getTransaccionclienteList().add(transaccioncliente);
			...
			if (idGiroactividadeconomicaOld != null && !idGiroactividadeconomicaOld.equals(idGiroactividadeconomicaNew)) {
				idGiroactividadeconomicaOld.getTransaccionclienteList().remove(transaccioncliente);
			...
			if (idGiroactividadeconomicaNew != null && !idGiroactividadeconomicaNew.equals(idGiroactividadeconomicaOld)) {
				idGiroactividadeconomicaNew.getTransaccionclienteList().add(transaccioncliente);
			...
			if (idTransaccionOld != null && !idTransaccionOld.equals(idTransaccionNew)) {
				idTransaccionOld.getTransaccionclienteList().remove(transaccioncliente);
				idTransaccionOld = em.merge(idTransaccionOld);
			}
			if (idTransaccionNew != null && !idTransaccionNew.equals(idTransaccionOld)) {
				idTransaccionNew.getTransaccionclienteList().add(transaccioncliente);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTransaccioncliente(id) == null) {
					throw new NonexistentEntityException("The transaccioncliente with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				transaccioncliente = em.getReference(Transaccioncliente.class, id);
				transaccioncliente.getId();
			...
			Cliente numeroIdentificacionCliente = transaccioncliente.getNumeroIdentificacionCliente();
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getTransaccionclienteList().remove(transaccioncliente);
				numeroIdentificacionCliente = em.merge(numeroIdentificacionCliente);
			}
			Giroactividadeconomica idGiroactividadeconomica = transaccioncliente.getIdGiroactividadeconomica();
			if (idGiroactividadeconomica != null) {
				idGiroactividadeconomica.getTransaccionclienteList().remove(transaccioncliente);
				idGiroactividadeconomica = em.merge(idGiroactividadeconomica);
			}
			Transaccion idTransaccion = transaccioncliente.getIdTransaccion();
			if (idTransaccion != null) {
				idTransaccion.getTransaccionclienteList().remove(transaccioncliente);
				idTransaccion = em.merge(idTransaccion);
			}
	 ...
	 * @return List<Transaccioncliente>
	 */
	public List<Transaccioncliente> findTransaccionclienteEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Transaccioncliente>
	 */
	private List<Transaccioncliente> findTransaccionclienteEntities(boolean all, int maxResults, int firstResult) {
