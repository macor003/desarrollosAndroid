...
public class PromocionarticuloJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Promocionarticulo promocionarticulo) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findPromocionarticulo(promocionarticulo.getId()) != null) {
				throw new PreexistingEntityException("Promocionarticulo " + promocionarticulo + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	...
	public void edit(Promocionarticulo promocionarticulo) throws NonexistentEntityException, Exception {
		...
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Promocionarticulo persistentPromocionarticulo = em.find(Promocionarticulo.class, promocionarticulo.getId());
			Articulo idArticuloOld = persistentPromocionarticulo.getIdArticulo();
			Articulo idArticuloNew = promocionarticulo.getIdArticulo();
			Promocion idPromocionOld = persistentPromocionarticulo.getIdPromocion();
			...
			if (idArticuloOld != null && !idArticuloOld.equals(idArticuloNew)) {
				idArticuloOld.getPromocionarticuloList().remove(promocionarticulo);
			...
			if (idPromocionOld != null && !idPromocionOld.equals(idPromocionNew)) {
				idPromocionOld.getPromocionarticuloList().remove(promocionarticulo);
				idPromocionOld = em.merge(idPromocionOld);
			}
			if (idPromocionNew != null && !idPromocionNew.equals(idPromocionOld)) {
				idPromocionNew.getPromocionarticuloList().add(promocionarticulo);
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			Promocion idPromocion = promocionarticulo.getIdPromocion();
			if (idPromocion != null) {
				idPromocion.getPromocionarticuloList().remove(promocionarticulo);
				idPromocion = em.merge(idPromocion);
			}
