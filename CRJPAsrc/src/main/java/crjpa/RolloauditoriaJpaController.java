...
public class RolloauditoriaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Rolloauditoria rolloauditoria) throws PreexistingEntityException, Exception {
		...
		try {
			...
			Serialimpresora idSerialimpresora = rolloauditoria.getIdSerialimpresora();
			...
			for (Transaccion transaccionListTransaccionToAttach : rolloauditoria.getTransaccionList()) {
			...
			for (Transaccion transaccionListTransaccion : rolloauditoria.getTransaccionList()) {
				...
				if (oldIdRolloauditoriaOfTransaccionListTransaccion != null) {
					oldIdRolloauditoriaOfTransaccionListTransaccion.getTransaccionList().remove(
			...
			if (findRolloauditoria(rolloauditoria.getId()) != null) {
				throw new PreexistingEntityException("Rolloauditoria " + rolloauditoria + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Rolloauditoria rolloauditoria) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Rolloauditoria persistentRolloauditoria = em.find(Rolloauditoria.class, rolloauditoria.getId());
			Serialimpresora idSerialimpresoraOld = persistentRolloauditoria.getIdSerialimpresora();
			Serialimpresora idSerialimpresoraNew = rolloauditoria.getIdSerialimpresora();
			Usuario idUsuarioOld = persistentRolloauditoria.getIdUsuario();
			Usuario idUsuarioNew = rolloauditoria.getIdUsuario();
			List<Transaccion> transaccionListOld = persistentRolloauditoria.getTransaccionList();
			List<Transaccion> transaccionListNew = rolloauditoria.getTransaccionList();
			...
			if (idSerialimpresoraOld != null && !idSerialimpresoraOld.equals(idSerialimpresoraNew)) {
				idSerialimpresoraOld.getRolloauditoriaList().remove(rolloauditoria);
			...
			if (idSerialimpresoraNew != null && !idSerialimpresoraNew.equals(idSerialimpresoraOld)) {
				idSerialimpresoraNew.getRolloauditoriaList().add(rolloauditoria);
			...
			for (Transaccion transaccionListNewTransaccion : transaccionListNew) {
				if (!transaccionListOld.contains(transaccionListNewTransaccion)) {
					Rolloauditoria oldIdRolloauditoriaOfTransaccionListNewTransaccion = transaccionListNewTransaccion
							.getIdRolloauditoria();
					transaccionListNewTransaccion.setIdRolloauditoria(rolloauditoria);
					...
					if (oldIdRolloauditoriaOfTransaccionListNewTransaccion != null
							&& !oldIdRolloauditoriaOfTransaccionListNewTransaccion.equals(rolloauditoria)) {
						oldIdRolloauditoriaOfTransaccionListNewTransaccion.getTransaccionList().remove(
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findRolloauditoria(id) == null) {
					throw new NonexistentEntityException("The rolloauditoria with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The rolloauditoria with id " + id + " no longer exists.", enfe);
			}
			Serialimpresora idSerialimpresora = rolloauditoria.getIdSerialimpresora();
			if (idSerialimpresora != null) {
				idSerialimpresora.getRolloauditoriaList().remove(rolloauditoria);
				idSerialimpresora = em.merge(idSerialimpresora);
			}
			Usuario idUsuario = rolloauditoria.getIdUsuario();
			if (idUsuario != null) {
			...
			List<Transaccion> transaccionList = rolloauditoria.getTransaccionList();
	 ...
	 * @return List<Rolloauditoria>
	 */
	public List<Rolloauditoria> findRolloauditoriaEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Rolloauditoria>
	 */
	private List<Rolloauditoria> findRolloauditoriaEntities(boolean all, int maxResults, int firstResult) {
