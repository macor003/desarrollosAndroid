...
public class PromocionlineaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Promocionlinea promocionlinea) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findPromocionlinea(promocionlinea.getId()) != null) {
				throw new PreexistingEntityException("Promocionlinea " + promocionlinea + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Promocionlinea promocionlinea) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Promocionlinea persistentPromocionlinea = em.find(Promocionlinea.class, promocionlinea.getId());
			Promocion idPromocionOld = persistentPromocionlinea.getIdPromocion();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPromocionlinea(id) == null) {
					throw new NonexistentEntityException("The promocionlinea with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The promocionlinea with id " + id + " no longer exists.", enfe);
			}
			Promocion idPromocion = promocionlinea.getIdPromocion();
			if (idPromocion != null) {
	 ...
	 * @return List<Promocionlinea>
	 */
	public List<Promocionlinea> findPromocionlineaEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Promocionlinea>
	 */
	private List<Promocionlinea> findPromocionlineaEntities(boolean all, int maxResults, int firstResult) {
