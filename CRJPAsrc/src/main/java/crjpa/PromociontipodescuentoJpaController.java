...
public class PromociontipodescuentoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Promociontipodescuento promociontipodescuento) throws PreexistingEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Tipodescuento idTipodescuento = promociontipodescuento.getIdTipodescuento();
			...
			if (idTipodescuento != null) {
				idTipodescuento.getPromociontipodescuentoList().add(promociontipodescuento);
				idTipodescuento = em.merge(idTipodescuento);
			}
			if (idPromocion != null) {
				idPromocion.getPromociontipodescuentoList().add(promociontipodescuento);
				idPromocion = em.merge(idPromocion);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findPromociontipodescuento(promociontipodescuento.getId()) != null) {
				throw new PreexistingEntityException("Promociontipodescuento " + promociontipodescuento
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Promociontipodescuento promociontipodescuento) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Promociontipodescuento persistentPromociontipodescuento = em.find(Promociontipodescuento.class,
					promociontipodescuento.getId());
			Tipodescuento idTipodescuentoOld = persistentPromociontipodescuento.getIdTipodescuento();
			Tipodescuento idTipodescuentoNew = promociontipodescuento.getIdTipodescuento();
			Promocion idPromocionOld = persistentPromociontipodescuento.getIdPromocion();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPromociontipodescuento(id) == null) {
					throw new NonexistentEntityException("The promociontipodescuento with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				promociontipodescuento = em.getReference(Promociontipodescuento.class, id);
				promociontipodescuento.getId();
			} catch (EntityNotFoundException enfe) {
				throw new NonexistentEntityException("The promociontipodescuento with id " + id + " no longer exists.",
						enfe);
			}
			Tipodescuento idTipodescuento = promociontipodescuento.getIdTipodescuento();
			if (idTipodescuento != null) {
				idTipodescuento.getPromociontipodescuentoList().remove(promociontipodescuento);
				idTipodescuento = em.merge(idTipodescuento);
			}
			Promocion idPromocion = promociontipodescuento.getIdPromocion();
			if (idPromocion != null) {
				idPromocion.getPromociontipodescuentoList().remove(promociontipodescuento);
				idPromocion = em.merge(idPromocion);
			}
	 ...
	 * @return List<Promociontipodescuento>
	 */
	public List<Promociontipodescuento> findPromociontipodescuentoEntities(int maxResults, int firstResult) {
		return findPromociontipodescuentoEntities(false, maxResults, firstResult);
	}
	 ...
	 * @return List<Promociontipodescuento>
	 */
	private List<Promociontipodescuento> findPromociontipodescuentoEntities(boolean all, int maxResults, int firstResult) {
	...
	public int getPromociontipodescuentoCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Promociontipodescuento> rt = cq.from(Promociontipodescuento.class);
