...
public class ArticulocodigoexternoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Articulocodigoexterno articulocodigoexterno) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (idArticulo != null) {
				idArticulo.getArticulocodigoexternoList().add(articulocodigoexterno);
				idArticulo = em.merge(idArticulo);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findArticulocodigoexterno(articulocodigoexterno.getId()) != null) {
				throw new PreexistingEntityException("Articulocodigoexterno " + articulocodigoexterno
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Articulocodigoexterno articulocodigoexterno) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Articulocodigoexterno persistentArticulocodigoexterno = em.find(Articulocodigoexterno.class,
					articulocodigoexterno.getId());
			Articulo idArticuloOld = persistentArticulocodigoexterno.getIdArticulo();
			...
			if (idArticuloOld != null && !idArticuloOld.equals(idArticuloNew)) {
				idArticuloOld.getArticulocodigoexternoList().remove(articulocodigoexterno);
				idArticuloOld = em.merge(idArticuloOld);
			}
			if (idArticuloNew != null && !idArticuloNew.equals(idArticuloOld)) {
				idArticuloNew.getArticulocodigoexternoList().add(articulocodigoexterno);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findArticulocodigoexterno(id) == null) {
					throw new NonexistentEntityException("The articulocodigoexterno with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				articulocodigoexterno = em.getReference(Articulocodigoexterno.class, id);
				articulocodigoexterno.getId();
			} catch (EntityNotFoundException enfe) {
				throw new NonexistentEntityException("The articulocodigoexterno with id " + id + " no longer exists.",
						enfe);
			}
			Articulo idArticulo = articulocodigoexterno.getIdArticulo();
			if (idArticulo != null) {
				idArticulo.getArticulocodigoexternoList().remove(articulocodigoexterno);
				idArticulo = em.merge(idArticulo);
			}
	 ...
	 * @return List<Articulocodigoexterno>
	 */
	public List<Articulocodigoexterno> findArticulocodigoexternoEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Articulocodigoexterno>
	 */
	private List<Articulocodigoexterno> findArticulocodigoexternoEntities(boolean all, int maxResults, int firstResult) {
	...
	public int getArticulocodigoexternoCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Articulocodigoexterno> rt = cq.from(Articulocodigoexterno.class);
