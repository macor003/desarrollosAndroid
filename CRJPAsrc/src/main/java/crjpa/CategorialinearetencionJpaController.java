...
public class CategorialinearetencionJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Categorialinearetencion categorialinearetencion) throws PreexistingEntityException, Exception {
...
//			Formadepago idFormadepago = categorialinearetencion.getIdFormadepago();
...
//			}
			...
			if (idLinea != null) {
				idLinea.getCategorialinearetencionList().add(categorialinearetencion);
				idLinea = em.merge(idLinea);
			}
//			if (idFormadepago != null) {
//				idFormadepago.getCategorialinearetencionList().add(categorialinearetencion);
//				idFormadepago = em.merge(idFormadepago);
//			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findCategorialinearetencion(categorialinearetencion.getId()) != null) {
				throw new PreexistingEntityException("Categorialinearetencion " + categorialinearetencion
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Categorialinearetencion categorialinearetencion) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Categorialinearetencion persistentCategorialinearetencion = em.find(Categorialinearetencion.class,
...
//			Formadepago idFormadepagoOld = persistentCategorialinearetencion.getIdFormadepago();
//			Formadepago idFormadepagoNew = categorialinearetencion.getIdFormadepago();
			if (idLineaNew != null) {
...
//			}
			...
			if (idLineaOld != null && !idLineaOld.equals(idLineaNew)) {
				idLineaOld.getCategorialinearetencionList().remove(categorialinearetencion);
				idLineaOld = em.merge(idLineaOld);
			}
			if (idLineaNew != null && !idLineaNew.equals(idLineaOld)) {
				idLineaNew.getCategorialinearetencionList().add(categorialinearetencion);
				idLineaNew = em.merge(idLineaNew);
			}
//			if (idFormadepagoOld != null && !idFormadepagoOld.equals(idFormadepagoNew)) {
//				idFormadepagoOld.getCategorialinearetencionList().remove(categorialinearetencion);
//				idFormadepagoOld = em.merge(idFormadepagoOld);
//			}
//			if (idFormadepagoNew != null && !idFormadepagoNew.equals(idFormadepagoOld)) {
//				idFormadepagoNew.getCategorialinearetencionList().add(categorialinearetencion);
...
//			}
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findCategorialinearetencion(id) == null) {
					throw new NonexistentEntityException("The categorialinearetencion with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				categorialinearetencion = em.getReference(Categorialinearetencion.class, id);
				categorialinearetencion.getId();
			} catch (EntityNotFoundException enfe) {
				throw new NonexistentEntityException(
						"The categorialinearetencion with id " + id + " no longer exists.", enfe);
			}
			Linea idLinea = categorialinearetencion.getIdLinea();
			if (idLinea != null) {
				idLinea.getCategorialinearetencionList().remove(categorialinearetencion);
				idLinea = em.merge(idLinea);
			}
//			Formadepago idFormadepago = categorialinearetencion.getIdFormadepago();
//			if (idFormadepago != null) {
//				idFormadepago.getCategorialinearetencionList().remove(categorialinearetencion);
//				idFormadepago = em.merge(idFormadepago);
//			}
	 ...
	 * @return List<Categorialinearetencion>
	 */
	public List<Categorialinearetencion> findCategorialinearetencionEntities(int maxResults, int firstResult) {
		return findCategorialinearetencionEntities(false, maxResults, firstResult);
	}
	 ...
	 * @return List<Categorialinearetencion>
	 */
	private List<Categorialinearetencion> findCategorialinearetencionEntities(boolean all, int maxResults,
			int firstResult) {
	...
	public int getCategorialinearetencionCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Categorialinearetencion> rt = cq.from(Categorialinearetencion.class);
