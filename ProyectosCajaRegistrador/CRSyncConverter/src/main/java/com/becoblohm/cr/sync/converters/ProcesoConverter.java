 ...
 */
public class ProcesoConverter implements ConverterInterface {
	...
	public Object fromServer(Object obj, String posType) {
		...
		crjpa400.Proceso serverValue = (crjpa400.Proceso) obj;
		crjpa.Proceso posValue = new Proceso();
		posValue.setDescripcion(serverValue.getDescripcion());
		posValue.setSerequiereautorizacion(String.valueOf(serverValue.getSerequiereautorizacion()));
		posValue.setEsautoautorizado(String.valueOf(serverValue.getEsautoautorizado()));
		posValue.setFecha(serverValue.getFecha());
		posValue.setId(serverValue.getId());
		posValue.setPermiteconciliacion(serverValue.getPermiteconciliacion());
		
		dm.setId(serverValue.getId().toString());

		dm.setEntity(posValue);
		dm.setSyncDate(serverValue.getUltimasincronizacion());
		dm.setTableName(crjpa.Proceso.class.getName().substring(crjpa.Proceso.class.getName().lastIndexOf(".") + 1));
