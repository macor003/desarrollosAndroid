...
public class TransaccionfaseJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Transaccionfase transaccionfase)
		...
		try {
			...
			if (findTransaccionfase(transaccionfase.getId()) != null) {
				throw new PreexistingEntityException("Transaccionfase "
						+ transaccionfase + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Transaccionfase transaccionfase)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Transaccionfase persistentTransaccionfase = em.find(
					Transaccionfase.class, transaccionfase.getId());
			Transaccion idTransaccionOld = persistentTransaccionfase
					.getIdTransaccion();
			...
			if (idTransaccionOld != null
				...
				idTransaccionOld.getTransaccionfaseList().remove(
						transaccionfase);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTransaccionfase(id) == null) {
					throw new NonexistentEntityException(
							"The transaccionfase with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	 ...
	 * @return List<Transaccionfase>
	 */
	public List<Transaccionfase> findTransaccionfaseEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Transaccionfase>
	 */
	private List<Transaccionfase> findTransaccionfaseEntities(boolean all,
