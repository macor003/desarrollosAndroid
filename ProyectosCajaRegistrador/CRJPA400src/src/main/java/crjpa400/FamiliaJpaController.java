...
public class FamiliaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Familia familia) throws PreexistingEntityException,
			Exception {
		...
		try {
			...
			for (Promocionfamilia promocionfamiliaListPromocionfamiliaToAttach : familia
			...
			for (Articulo articuloListArticuloToAttach : familia
					.getArticuloList()) {
			...
			for (Promocionfamilia promocionfamiliaListPromocionfamilia : familia
					.getPromocionfamiliaList()) {
				...
				if (oldIdFamiliaOfPromocionfamiliaListPromocionfamilia != null) {
					oldIdFamiliaOfPromocionfamiliaListPromocionfamilia
			...
			for (Articulo articuloListArticulo : familia.getArticuloList()) {
				Familia oldIdFamiliaOfArticuloListArticulo = articuloListArticulo
						.getIdFamilia();
				...
				if (oldIdFamiliaOfArticuloListArticulo != null) {
					oldIdFamiliaOfArticuloListArticulo.getArticuloList()
							.remove(articuloListArticulo);
					oldIdFamiliaOfArticuloListArticulo = em
							.merge(oldIdFamiliaOfArticuloListArticulo);
				}
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findFamilia(familia.getId()) != null) {
				throw new PreexistingEntityException("Familia " + familia
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Familia familia) throws IllegalOrphanException,
		...
		try {
			...
			List<Promocionfamilia> promocionfamiliaListOld = persistentFamilia
					.getPromocionfamiliaList();
			List<Promocionfamilia> promocionfamiliaListNew = familia
					.getPromocionfamiliaList();
			List<Articulo> articuloListOld = persistentFamilia
					.getArticuloList();
				...
				if (!promocionfamiliaListNew
					...
					illegalOrphanMessages
							.add("You must retain Promocionfamilia "
									...
									+ " since its idFamilia field is not nullable.");
				}
			}
			if (illegalOrphanMessages != null) {
			...
			for (Promocionfamilia promocionfamiliaListNewPromocionfamilia : promocionfamiliaListNew) {
				if (!promocionfamiliaListOld
					...
					if (oldIdFamiliaOfPromocionfamiliaListNewPromocionfamilia != null
							&& !oldIdFamiliaOfPromocionfamiliaListNewPromocionfamilia
									.equals(familia)) {
						oldIdFamiliaOfPromocionfamiliaListNewPromocionfamilia
								.getPromocionfamiliaList()
			...
			for (Articulo articuloListNewArticulo : articuloListNew) {
				if (!articuloListOld.contains(articuloListNewArticulo)) {
					Familia oldIdFamiliaOfArticuloListNewArticulo = articuloListNewArticulo
							.getIdFamilia();
					...
					if (oldIdFamiliaOfArticuloListNewArticulo != null
							&& !oldIdFamiliaOfArticuloListNewArticulo
									.equals(familia)) {
						oldIdFamiliaOfArticuloListNewArticulo.getArticuloList()
								.remove(articuloListNewArticulo);
						oldIdFamiliaOfArticuloListNewArticulo = em
								.merge(oldIdFamiliaOfArticuloListNewArticulo);
					}
				}
			}
			em.getTransaction().commit();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findFamilia(id) == null) {
					throw new NonexistentEntityException("The familia with id "
							+ id + " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws IllegalOrphanException,
		...
		try {
			...
			List<String> illegalOrphanMessages = null;
			List<Promocionfamilia> promocionfamiliaListOrphanCheck = familia
					.getPromocionfamiliaList();
	 ...
	 * @return List<Familia>
	 */
	private List<Familia> findFamiliaEntities(boolean all, int maxResults,
			int firstResult) {
