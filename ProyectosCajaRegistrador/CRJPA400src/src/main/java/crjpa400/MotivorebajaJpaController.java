...
public class MotivorebajaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Motivorebaja motivorebaja)
		...
		try {
			...
			if (findMotivorebaja(motivorebaja.getId()) != null) {
				throw new PreexistingEntityException("Motivorebaja "
						+ motivorebaja + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Motivorebaja motivorebaja)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Motivorebaja persistentMotivorebaja = em.find(Motivorebaja.class,
					motivorebaja.getId());
			Tipodescuento idTipodescuentoOld = persistentMotivorebaja
					.getIdTipodescuento();
			Tipodescuento idTipodescuentoNew = motivorebaja
					.getIdTipodescuento();
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException(
						"The motivorebaja with id " + id + " no longer exists.",
						enfe);
			}
			Tipodescuento idTipodescuento = motivorebaja.getIdTipodescuento();
			if (idTipodescuento != null) {
	 ...
	 * @return List<Motivorebaja>
	 */
	public List<Motivorebaja> findMotivorebajaEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Motivorebaja>
	 */
	private List<Motivorebaja> findMotivorebajaEntities(boolean all,
