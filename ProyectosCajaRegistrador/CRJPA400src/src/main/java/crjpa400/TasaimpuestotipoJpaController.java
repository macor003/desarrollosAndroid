...
public class TasaimpuestotipoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Tasaimpuestotipo tasaimpuestotipo)
		...
		if (tasaimpuestotipo.getTasaimpuestovalorList() == null) {
			tasaimpuestotipo
					.setTasaimpuestovalorList(new ArrayList<Tasaimpuestovalor>());
		}
		...
		try {
			...
			for (Articulo articuloListArticuloToAttach : tasaimpuestotipo
					.getArticuloList()) {
			...
			for (Tasaimpuestovalor tasaimpuestovalorListTasaimpuestovalorToAttach : tasaimpuestotipo
				...
				tasaimpuestovalorListTasaimpuestovalorToAttach = em
						.getReference(
								tasaimpuestovalorListTasaimpuestovalorToAttach
										.getClass(),
								tasaimpuestovalorListTasaimpuestovalorToAttach
										.getId());
				attachedTasaimpuestovalorList
						.add(tasaimpuestovalorListTasaimpuestovalorToAttach);
			}
			tasaimpuestotipo
					.setTasaimpuestovalorList(attachedTasaimpuestovalorList);
			em.persist(tasaimpuestotipo);
			for (Articulo articuloListArticulo : tasaimpuestotipo
					.getArticuloList()) {
				...
				if (oldIdTasaimpuestotipoOfArticuloListArticulo != null) {
					oldIdTasaimpuestotipoOfArticuloListArticulo
							.getArticuloList().remove(articuloListArticulo);
					oldIdTasaimpuestotipoOfArticuloListArticulo = em
							.merge(oldIdTasaimpuestotipoOfArticuloListArticulo);
				}
			}
			for (Tasaimpuestovalor tasaimpuestovalorListTasaimpuestovalor : tasaimpuestotipo
					.getTasaimpuestovalorList()) {
				...
				tasaimpuestovalorListTasaimpuestovalor
						.setIdTasaimpuestotipo(tasaimpuestotipo);
				tasaimpuestovalorListTasaimpuestovalor = em
						.merge(tasaimpuestovalorListTasaimpuestovalor);
				if (oldIdTasaimpuestotipoOfTasaimpuestovalorListTasaimpuestovalor != null) {
					oldIdTasaimpuestotipoOfTasaimpuestovalorListTasaimpuestovalor
							.getTasaimpuestovalorList().remove(
									tasaimpuestovalorListTasaimpuestovalor);
			...
			if (findTasaimpuestotipo(tasaimpuestotipo.getId()) != null) {
				throw new PreexistingEntityException("Tasaimpuestotipo "
						+ tasaimpuestotipo + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Tasaimpuestotipo tasaimpuestotipo)
		...
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Tasaimpuestotipo persistentTasaimpuestotipo = em.find(
					Tasaimpuestotipo.class, tasaimpuestotipo.getId());
			List<Articulo> articuloListOld = persistentTasaimpuestotipo
					.getArticuloList();
			List<Articulo> articuloListNew = tasaimpuestotipo.getArticuloList();
			List<Tasaimpuestovalor> tasaimpuestovalorListOld = persistentTasaimpuestotipo
					.getTasaimpuestovalorList();
			List<Tasaimpuestovalor> tasaimpuestovalorListNew = tasaimpuestotipo
					.getTasaimpuestovalorList();
			...
			for (Tasaimpuestovalor tasaimpuestovalorListOldTasaimpuestovalor : tasaimpuestovalorListOld) {
				if (!tasaimpuestovalorListNew
						.contains(tasaimpuestovalorListOldTasaimpuestovalor)) {
					if (illegalOrphanMessages == null) {
						illegalOrphanMessages = new ArrayList<String>();
					}
					illegalOrphanMessages
							.add("You must retain Tasaimpuestovalor "
			...
			for (Tasaimpuestovalor tasaimpuestovalorListNewTasaimpuestovalorToAttach : tasaimpuestovalorListNew) {
				tasaimpuestovalorListNewTasaimpuestovalorToAttach = em
						.getReference(
								tasaimpuestovalorListNewTasaimpuestovalorToAttach
										.getClass(),
								tasaimpuestovalorListNewTasaimpuestovalorToAttach
										.getId());
				attachedTasaimpuestovalorListNew
						.add(tasaimpuestovalorListNewTasaimpuestovalorToAttach);
			}
			...
			for (Articulo articuloListNewArticulo : articuloListNew) {
				if (!articuloListOld.contains(articuloListNewArticulo)) {
					...
					if (oldIdTasaimpuestotipoOfArticuloListNewArticulo != null
							&& !oldIdTasaimpuestotipoOfArticuloListNewArticulo
			...
			for (Tasaimpuestovalor tasaimpuestovalorListNewTasaimpuestovalor : tasaimpuestovalorListNew) {
				if (!tasaimpuestovalorListOld
						.contains(tasaimpuestovalorListNewTasaimpuestovalor)) {
					Tasaimpuestotipo oldIdTasaimpuestotipoOfTasaimpuestovalorListNewTasaimpuestovalor = tasaimpuestovalorListNewTasaimpuestovalor
							.getIdTasaimpuestotipo();
					tasaimpuestovalorListNewTasaimpuestovalor
							.setIdTasaimpuestotipo(tasaimpuestotipo);
					tasaimpuestovalorListNewTasaimpuestovalor = em
							.merge(tasaimpuestovalorListNewTasaimpuestovalor);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTasaimpuestotipo(id) == null) {
					throw new NonexistentEntityException(
							"The tasaimpuestotipo with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws IllegalOrphanException,
		...
		try {
			...
			List<String> illegalOrphanMessages = null;
			List<Articulo> articuloListOrphanCheck = tasaimpuestotipo
					.getArticuloList();
			...
			List<Tasaimpuestovalor> tasaimpuestovalorListOrphanCheck = tasaimpuestotipo
					.getTasaimpuestovalorList();
	 ...
	 * @return List<Tasaimpuestotipo>
	 */
	public List<Tasaimpuestotipo> findTasaimpuestotipoEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Tasaimpuestotipo>
	 */
	private List<Tasaimpuestotipo> findTasaimpuestotipoEntities(boolean all,
