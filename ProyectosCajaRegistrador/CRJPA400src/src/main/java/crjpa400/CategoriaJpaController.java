...
public class CategoriaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Categoria categoria) throws PreexistingEntityException,
			Exception {
		...
		try {
			...
			for (Linea lineaListLineaToAttach : categoria.getLineaList()) {
				lineaListLineaToAttach = em.getReference(
						lineaListLineaToAttach.getClass(),
			...
			for (Linea lineaListLinea : categoria.getLineaList()) {
				Categoria oldIdCategoriaOfLineaListLinea = lineaListLinea
						.getIdCategoria();
				lineaListLinea.setIdCategoria(categoria);
				lineaListLinea = em.merge(lineaListLinea);
				if (oldIdCategoriaOfLineaListLinea != null) {
					oldIdCategoriaOfLineaListLinea.getLineaList().remove(
							lineaListLinea);
					oldIdCategoriaOfLineaListLinea = em
							.merge(oldIdCategoriaOfLineaListLinea);
				}
			}
			for (Promocioncategoria promocioncategoriaListPromocioncategoria : categoria
					.getPromocioncategoriaList()) {
				...
				if (oldIdCategoriaOfPromocioncategoriaListPromocioncategoria != null) {
					oldIdCategoriaOfPromocioncategoriaListPromocioncategoria
			...
			if (findCategoria(categoria.getId()) != null) {
				throw new PreexistingEntityException("Categoria " + categoria
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Categoria categoria) throws IllegalOrphanException,
			NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Categoria persistentCategoria = em.find(Categoria.class,
					categoria.getId());
			List<Linea> lineaListOld = persistentCategoria.getLineaList();
			List<Linea> lineaListNew = categoria.getLineaList();
			List<Promocioncategoria> promocioncategoriaListOld = persistentCategoria
					.getPromocioncategoriaList();
			List<Promocioncategoria> promocioncategoriaListNew = categoria
					.getPromocioncategoriaList();
			...
			for (Linea lineaListOldLinea : lineaListOld) {
				if (!lineaListNew.contains(lineaListOldLinea)) {
					...
					illegalOrphanMessages.add("You must retain Linea "
							+ lineaListOldLinea
			...
			for (Linea lineaListNewLineaToAttach : lineaListNew) {
				lineaListNewLineaToAttach = em.getReference(
						lineaListNewLineaToAttach.getClass(),
			...
			for (Linea lineaListNewLinea : lineaListNew) {
				if (!lineaListOld.contains(lineaListNewLinea)) {
					Categoria oldIdCategoriaOfLineaListNewLinea = lineaListNewLinea
							.getIdCategoria();
					...
					if (oldIdCategoriaOfLineaListNewLinea != null
							&& !oldIdCategoriaOfLineaListNewLinea
									.equals(categoria)) {
						oldIdCategoriaOfLineaListNewLinea.getLineaList()
								.remove(lineaListNewLinea);
						oldIdCategoriaOfLineaListNewLinea = em
								.merge(oldIdCategoriaOfLineaListNewLinea);
					}
				}
			}
			for (Promocioncategoria promocioncategoriaListNewPromocioncategoria : promocioncategoriaListNew) {
				if (!promocioncategoriaListOld
					...
					if (oldIdCategoriaOfPromocioncategoriaListNewPromocioncategoria != null
							&& !oldIdCategoriaOfPromocioncategoriaListNewPromocioncategoria
									.equals(categoria)) {
						oldIdCategoriaOfPromocioncategoriaListNewPromocioncategoria
								.getPromocioncategoriaList()
	...
	public void destroy(Long id) throws IllegalOrphanException,
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The categoria with id "
			...
			for (Linea lineaListOrphanCheckLinea : lineaListOrphanCheck) {
				...
				illegalOrphanMessages
						.add("This Categoria ("
								+ categoria
								+ ") cannot be destroyed since the Linea "
								+ lineaListOrphanCheckLinea
								+ " in its lineaList field has a non-nullable idCategoria field.");
			}
			List<Promocioncategoria> promocioncategoriaListOrphanCheck = categoria
					.getPromocioncategoriaList();
	 ...
	 * @return List<Categoria>
	 */
	private List<Categoria> findCategoriaEntities(boolean all, int maxResults,
			int firstResult) {
