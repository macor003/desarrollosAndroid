...
public class EntregaformadepagoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Entregaformadepago entregaformadepago)
		...
		try {
			...
			Monedadenominacion idMonedadenominacion = entregaformadepago
					.getIdMonedadenominacion();
			if (idMonedadenominacion != null) {
				idMonedadenominacion = em.getReference(
						idMonedadenominacion.getClass(),
						idMonedadenominacion.getId());
				entregaformadepago
						.setIdMonedadenominacion(idMonedadenominacion);
			}
			Entrega idEntrega = entregaformadepago.getIdEntrega();
			if (idEntrega != null) {
				idEntrega = em.getReference(idEntrega.getClass(),
						idEntrega.getId());
				entregaformadepago.setIdEntrega(idEntrega);
			}
			em.persist(entregaformadepago);
			if (idFormadepago != null) {
				idFormadepago.getEntregaformadepagoList().add(
						entregaformadepago);
				idFormadepago = em.merge(idFormadepago);
			}
			if (idMonedadenominacion != null) {
				idMonedadenominacion.getEntregaformadepagoList().add(
						entregaformadepago);
			...
			if (findEntregaformadepago(entregaformadepago.getId()) != null) {
				throw new PreexistingEntityException("Entregaformadepago "
						+ entregaformadepago + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Entregaformadepago entregaformadepago)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Entregaformadepago persistentEntregaformadepago = em.find(
					Entregaformadepago.class, entregaformadepago.getId());
			Formadepago idFormadepagoOld = persistentEntregaformadepago
					.getIdFormadepago();
			Formadepago idFormadepagoNew = entregaformadepago
					.getIdFormadepago();
			Monedadenominacion idMonedadenominacionOld = persistentEntregaformadepago
					.getIdMonedadenominacion();
			Monedadenominacion idMonedadenominacionNew = entregaformadepago
					.getIdMonedadenominacion();
			...
			if (idMonedadenominacionNew != null) {
				idMonedadenominacionNew = em.getReference(
						idMonedadenominacionNew.getClass(),
						idMonedadenominacionNew.getId());
				entregaformadepago
						.setIdMonedadenominacion(idMonedadenominacionNew);
			}
			if (idEntregaNew != null) {
				idEntregaNew = em.getReference(idEntregaNew.getClass(),
						idEntregaNew.getId());
			...
			if (idFormadepagoOld != null
				...
				idFormadepagoOld.getEntregaformadepagoList().remove(
						entregaformadepago);
				idFormadepagoOld = em.merge(idFormadepagoOld);
			}
			if (idFormadepagoNew != null
					&& !idFormadepagoNew.equals(idFormadepagoOld)) {
				idFormadepagoNew.getEntregaformadepagoList().add(
						entregaformadepago);
				idFormadepagoNew = em.merge(idFormadepagoNew);
			}
			if (idMonedadenominacionOld != null
					&& !idMonedadenominacionOld.equals(idMonedadenominacionNew)) {
				idMonedadenominacionOld.getEntregaformadepagoList().remove(
						entregaformadepago);
				idMonedadenominacionOld = em.merge(idMonedadenominacionOld);
			}
			if (idMonedadenominacionNew != null
					&& !idMonedadenominacionNew.equals(idMonedadenominacionOld)) {
				idMonedadenominacionNew.getEntregaformadepagoList().add(
						entregaformadepago);
				idMonedadenominacionNew = em.merge(idMonedadenominacionNew);
			}
			if (idEntregaOld != null && !idEntregaOld.equals(idEntregaNew)) {
				idEntregaOld.getEntregaformadepagoList().remove(
						entregaformadepago);
				idEntregaOld = em.merge(idEntregaOld);
			}
			if (idEntregaNew != null && !idEntregaNew.equals(idEntregaOld)) {
				idEntregaNew.getEntregaformadepagoList()
						.add(entregaformadepago);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findEntregaformadepago(id) == null) {
					throw new NonexistentEntityException(
							"The entregaformadepago with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				entregaformadepago = em.getReference(Entregaformadepago.class,
						id);
			...
			Formadepago idFormadepago = entregaformadepago.getIdFormadepago();
			if (idFormadepago != null) {
				idFormadepago.getEntregaformadepagoList().remove(
						entregaformadepago);
				idFormadepago = em.merge(idFormadepago);
			}
			Monedadenominacion idMonedadenominacion = entregaformadepago
					.getIdMonedadenominacion();
			if (idMonedadenominacion != null) {
				idMonedadenominacion.getEntregaformadepagoList().remove(
						entregaformadepago);
				idMonedadenominacion = em.merge(idMonedadenominacion);
			}
			Entrega idEntrega = entregaformadepago.getIdEntrega();
			if (idEntrega != null) {
				idEntrega.getEntregaformadepagoList()
						.remove(entregaformadepago);
				idEntrega = em.merge(idEntrega);
			}
	 ...
	 * @return List<Entregaformadepago>
	 */
	private List<Entregaformadepago> findEntregaformadepagoEntities(
