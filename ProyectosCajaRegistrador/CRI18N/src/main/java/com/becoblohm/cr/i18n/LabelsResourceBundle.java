 ...
 */
public class LabelsResourceBundle extends ListResourceBundle {
	...
	private static final Object[][] contents = {
			...
			{ "global.originaltransactionid", "Id de transaccion original" }, { "global.storenumber", "Tienda" },
			...
			/* Botones */
			{ "action.ok", "Aceptar" }, { "action.cancel", "Cancelar" }, { "action.verify", "Verificar" },
			{ "action.print", "Imprimir" }, { "action.delete", "Eliminar" }, { "action.pay", "Pagar" },
			{ "action.delivery", "Condición de Entrega" }, { "action.cashier", "Cajero" },
			{ "action.auth", "Autorizante" },

			/* Ventana diferencias en entrega total */
			{ "action.cashier.difference.pm", "Forma de Pago" },
			{ "action.cashier.difference.processed", "Procesadas" },
			{ "action.cashier.difference.charged", "Cargadas" },
			{ "action.cashier.difference.differenceQuantity", "Cantidad Diferencia" },
			{ "action.cashier.difference.chargedAmount", "Monto Cargado" },
			{ "action.cashier.difference.differenceAmount", "Monto Diferencia" },
			...
			{ "cashier.delivery", "<html>Reporte de <br /> Cajero</html>" },
			{ "cashier.deliverytitle", "Reporte de Cajero" },
			...
			{ "retention.name", "Retención de impuesto" }, { "retention.number", "Número de comprobante" },
			...
			/* Etiquetas del Modulo TaxDeduction */
			{ "taxdeduction.errorzeroarticles",
					"No se puede aplicar exención de impuestos" + System.getProperty("line.separator")
							+ "si no hay artículos cargados" },
			...
			{ "paymentmeth.vposuse", "¿Desea usar el Punto Ágil?" }, { "paymentmeth.approved", "Transacción Aprobada" },
			{ "paymentmeth.rejected", "Transacción Rechazada" },
			 ...
			 * Modulo de auditoria
			 */
			{ "audit.name", "Registro de auditoría" }, { "audit.module", "Módulo" },
			{ "audit.processid", "ID Proceso" }, { "audit.funtion", "Función" }, { "audit.postype", "Tipo de caja" },
			{ "audit.fingerid", "IdHuella" }, { "audit.password", "Clave" },
			{ "audit.deletedrecords", "Registros Depurados" }, { "audit.idauth", "Id Autorizante" },
			{ "audit.idfdp", "Id Forma de Pago" }, { "audit.idtrans", "Id Transacción" }, { "audit.amount", "Monto" },
			...
			{ "check.number", "Nro Cheque (8 digitos)" }, { "check.accountnumber", "Nro Cuenta (20 digitos)" },
			...
			{ "creditsepa.cardNumber", "N° tarjeta" }, { "creditsepa.cardNumberCorporate", "Número de identificación" },
			...
			{ "closureday.printigZReport", "Emitiendo reporte Z" }, { "closureday.closingvpos", "Cerrando punto ágil" },
			{ "closureday.closingsession", "Cerrando sesión" }, { "closureday.cleaningdata", "Depurando data" },
			...
			{ "messages.nullclient", "Identificador no encontrado o con formato inválido" },
			{ "messages.nullclient_addclient",
					"Identificador no encontrado o con formato inválido" + System.getProperty("line.separator")
							+ "Por favor asignar cliente" },
			{ "messages.nongenericcustomer", "Ingrese un cliente distinto del generico para continuar" },
			 ...
			 * Pantalla Articulo
			 */
			{ "retail.article", "Artículo" }, { "retail.description", "Descripción" },
			{ "retail.quantity", "Cantidad" }, { "retail.preciofinal", "Precio final" },
			{ "retail.subtotal", "Sub Total" }, { "retail.discount", "Descuento" }, { "retail.entrega", "Entrega" },
