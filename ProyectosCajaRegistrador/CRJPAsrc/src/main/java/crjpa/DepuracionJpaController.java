...
public class DepuracionJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Depuracion depuracion) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findDepuracion(depuracion.getId()) != null) {
				throw new PreexistingEntityException("Depuracion " + depuracion + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Depuracion depuracion) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findDepuracion(id) == null) {
					throw new NonexistentEntityException("The depuracion with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The depuracion with id " + id + " no longer exists.", enfe);
			}
	 ...
	 * @return List<Depuracion>
	 */
	public List<Depuracion> findDepuracionEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Depuracion>
	 */
	private List<Depuracion> findDepuracionEntities(boolean all, int maxResults, int firstResult) {
