...
public class CategorialineaincluyeJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Categorialineaincluye categorialineaincluye) throws PreexistingEntityException, Exception {
		...
		try {
			...
			Giroactividadeconomica idGiroactividadeconomica = categorialineaincluye.getIdGiroactividadeconomica();
			if (idGiroactividadeconomica != null) {
				...
				categorialineaincluye.setIdGiroactividadeconomica(idGiroactividadeconomica);
			}
			Tipodocumento idTipodocumento = categorialineaincluye.getIdTipodocumento();
			...
			if (idLinea != null) {
				idLinea.getCategorialineaincluyeList().add(categorialineaincluye);
				idLinea = em.merge(idLinea);
			}
			if (idGiroactividadeconomica != null) {
				idGiroactividadeconomica.getCategorialineaincluyeList().add(categorialineaincluye);
				idGiroactividadeconomica = em.merge(idGiroactividadeconomica);
			}
			if (idTipodocumento != null) {
				idTipodocumento.getCategorialineaincluyeList().add(categorialineaincluye);
				idTipodocumento = em.merge(idTipodocumento);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findCategorialineaincluye(categorialineaincluye.getId()) != null) {
				throw new PreexistingEntityException("Categorialineaincluye " + categorialineaincluye
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Categorialineaincluye categorialineaincluye) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Categorialineaincluye persistentCategorialineaincluye = em.find(Categorialineaincluye.class,
			...
			Giroactividadeconomica idGiroactividadeconomicaNew = categorialineaincluye.getIdGiroactividadeconomica();
			Tipodocumento idTipodocumentoOld = persistentCategorialineaincluye.getIdTipodocumento();
			Tipodocumento idTipodocumentoNew = categorialineaincluye.getIdTipodocumento();
			if (idLineaNew != null) {
			...
			if (idGiroactividadeconomicaNew != null) {
				...
				categorialineaincluye.setIdGiroactividadeconomica(idGiroactividadeconomicaNew);
			}
			...
			if (idLineaOld != null && !idLineaOld.equals(idLineaNew)) {
				idLineaOld.getCategorialineaincluyeList().remove(categorialineaincluye);
				idLineaOld = em.merge(idLineaOld);
			}
			if (idLineaNew != null && !idLineaNew.equals(idLineaOld)) {
				idLineaNew.getCategorialineaincluyeList().add(categorialineaincluye);
			...
			if (idGiroactividadeconomicaOld != null && !idGiroactividadeconomicaOld.equals(idGiroactividadeconomicaNew)) {
				idGiroactividadeconomicaOld.getCategorialineaincluyeList().remove(categorialineaincluye);
			...
			if (idGiroactividadeconomicaNew != null && !idGiroactividadeconomicaNew.equals(idGiroactividadeconomicaOld)) {
				idGiroactividadeconomicaNew.getCategorialineaincluyeList().add(categorialineaincluye);
			...
			if (idTipodocumentoOld != null && !idTipodocumentoOld.equals(idTipodocumentoNew)) {
				idTipodocumentoOld.getCategorialineaincluyeList().remove(categorialineaincluye);
				idTipodocumentoOld = em.merge(idTipodocumentoOld);
			}
			if (idTipodocumentoNew != null && !idTipodocumentoNew.equals(idTipodocumentoOld)) {
				idTipodocumentoNew.getCategorialineaincluyeList().add(categorialineaincluye);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findCategorialineaincluye(id) == null) {
					throw new NonexistentEntityException("The categorialineaincluye with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				categorialineaincluye = em.getReference(Categorialineaincluye.class, id);
				categorialineaincluye.getId();
			} catch (EntityNotFoundException enfe) {
				throw new NonexistentEntityException("The categorialineaincluye with id " + id + " no longer exists.",
						enfe);
			}
			Linea idLinea = categorialineaincluye.getIdLinea();
			if (idLinea != null) {
				idLinea.getCategorialineaincluyeList().remove(categorialineaincluye);
				idLinea = em.merge(idLinea);
			}
			Giroactividadeconomica idGiroactividadeconomica = categorialineaincluye.getIdGiroactividadeconomica();
			if (idGiroactividadeconomica != null) {
				idGiroactividadeconomica.getCategorialineaincluyeList().remove(categorialineaincluye);
				idGiroactividadeconomica = em.merge(idGiroactividadeconomica);
			}
			Tipodocumento idTipodocumento = categorialineaincluye.getIdTipodocumento();
			if (idTipodocumento != null) {
				idTipodocumento.getCategorialineaincluyeList().remove(categorialineaincluye);
				idTipodocumento = em.merge(idTipodocumento);
			}
	 ...
	 * @return List<Categorialineaincluye>
	 */
	public List<Categorialineaincluye> findCategorialineaincluyeEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Categorialineaincluye>
	 */
	private List<Categorialineaincluye> findCategorialineaincluyeEntities(boolean all, int maxResults, int firstResult) {
	...
	public int getCategorialineaincluyeCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Categorialineaincluye> rt = cq.from(Categorialineaincluye.class);
