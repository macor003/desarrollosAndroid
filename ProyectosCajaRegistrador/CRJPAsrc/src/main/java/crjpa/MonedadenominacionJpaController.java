...
public class MonedadenominacionJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Monedadenominacion monedadenominacion) throws PreexistingEntityException, Exception {
		if (monedadenominacion.getEntregaformadepagoList() == null) {
			monedadenominacion.setEntregaformadepagoList(new ArrayList<Entregaformadepago>());
		}
		...
		try {
			...
			monedadenominacion.setEntregaformadepagoList(attachedEntregaformadepagoList);
			...
			for (Entregaformadepago entregaformadepagoListEntregaformadepago : monedadenominacion
				...
				entregaformadepagoListEntregaformadepago.setIdMonedadenominacion(monedadenominacion);
				...
				if (oldIdMonedadenominacionOfEntregaformadepagoListEntregaformadepago != null) {
					oldIdMonedadenominacionOfEntregaformadepagoListEntregaformadepago.getEntregaformadepagoList()
							.remove(entregaformadepagoListEntregaformadepago);
			...
			if (findMonedadenominacion(monedadenominacion.getId()) != null) {
				throw new PreexistingEntityException("Monedadenominacion " + monedadenominacion + " already exists.",
						ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Monedadenominacion monedadenominacion) throws IllegalOrphanException, NonexistentEntityException,
			Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Monedadenominacion persistentMonedadenominacion = em.find(Monedadenominacion.class,
			...
			List<Entregaformadepago> entregaformadepagoListOld = persistentMonedadenominacion
					.getEntregaformadepagoList();
			List<Entregaformadepago> entregaformadepagoListNew = monedadenominacion.getEntregaformadepagoList();
			...
			monedadenominacion.setEntregaformadepagoList(entregaformadepagoListNew);
			monedadenominacion = em.merge(monedadenominacion);
			if (idMonedaOld != null && !idMonedaOld.equals(idMonedaNew)) {
				idMonedaOld.getMonedadenominacionList().remove(monedadenominacion);
			...
			for (Entregaformadepago entregaformadepagoListNewEntregaformadepago : entregaformadepagoListNew) {
				if (!entregaformadepagoListOld.contains(entregaformadepagoListNewEntregaformadepago)) {
					Monedadenominacion oldIdMonedadenominacionOfEntregaformadepagoListNewEntregaformadepago = entregaformadepagoListNewEntregaformadepago
							.getIdMonedadenominacion();
					entregaformadepagoListNewEntregaformadepago.setIdMonedadenominacion(monedadenominacion);
					...
					if (oldIdMonedadenominacionOfEntregaformadepagoListNewEntregaformadepago != null
						...
						oldIdMonedadenominacionOfEntregaformadepagoListNewEntregaformadepago
								.getEntregaformadepagoList().remove(entregaformadepagoListNewEntregaformadepago);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findMonedadenominacion(id) == null) {
					throw new NonexistentEntityException("The monedadenominacion with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			try {
				monedadenominacion = em.getReference(Monedadenominacion.class, id);
				monedadenominacion.getId();
			...
			List<String> illegalOrphanMessages = null;
			List<Entregaformadepago> entregaformadepagoListOrphanCheck = monedadenominacion.getEntregaformadepagoList();
			for (Entregaformadepago entregaformadepagoListOrphanCheckEntregaformadepago : entregaformadepagoListOrphanCheck) {
				if (illegalOrphanMessages == null) {
					illegalOrphanMessages = new ArrayList<String>();
				}
				illegalOrphanMessages.add("This Monedadenominacion (" + monedadenominacion
	 ...
	 * @return List<Monedadenominacion>
	 */
	public List<Monedadenominacion> findMonedadenominacionEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Monedadenominacion>
	 */
	private List<Monedadenominacion> findMonedadenominacionEntities(boolean all, int maxResults, int firstResult) {
