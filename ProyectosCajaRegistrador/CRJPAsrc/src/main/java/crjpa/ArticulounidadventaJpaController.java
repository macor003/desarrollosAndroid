...
public class ArticulounidadventaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Articulounidadventa articulounidadventa) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (idUnidadventa != null) {
				// idUnidadventa.getArticulounidadventaList().add(articulounidadventa);
				idUnidadventa = em.merge(idUnidadventa);
			}
			if (idArticulo != null) {
				idArticulo.getArticulounidadventaList().add(articulounidadventa);
				idArticulo = em.merge(idArticulo);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findArticulounidadventa(articulounidadventa.getId()) != null) {
				throw new PreexistingEntityException("Articulounidadventa " + articulounidadventa + " already exists.",
						ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Articulounidadventa articulounidadventa) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Articulounidadventa persistentArticulounidadventa = em.find(Articulounidadventa.class,
					articulounidadventa.getId());
			Unidadventa idUnidadventaOld = persistentArticulounidadventa.getIdUnidadventa();
			Unidadventa idUnidadventaNew = articulounidadventa.getIdUnidadventa();
			Articulo idArticuloOld = persistentArticulounidadventa.getIdArticulo();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findArticulounidadventa(id) == null) {
					throw new NonexistentEntityException("The articulounidadventa with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				articulounidadventa = em.getReference(Articulounidadventa.class, id);
				articulounidadventa.getId();
			...
			Unidadventa idUnidadventa = articulounidadventa.getIdUnidadventa();
			if (idUnidadventa != null) {
				// idUnidadventa.getArticulounidadventaList().remove(articulounidadventa);
				idUnidadventa = em.merge(idUnidadventa);
			}
			Articulo idArticulo = articulounidadventa.getIdArticulo();
			if (idArticulo != null) {
				idArticulo.getArticulounidadventaList().remove(articulounidadventa);
				idArticulo = em.merge(idArticulo);
			}
	 ...
	 * @return List<Articulounidadventa>
	 */
	public List<Articulounidadventa> findArticulounidadventaEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Articulounidadventa>
	 */
	private List<Articulounidadventa> findArticulounidadventaEntities(boolean all, int maxResults, int firstResult) {
