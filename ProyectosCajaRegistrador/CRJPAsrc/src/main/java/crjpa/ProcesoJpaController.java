...
public class ProcesoJpaController implements Serializable {
	...
	public void create(Proceso proceso) throws PreexistingEntityException, Exception {
		...
		try {
			...
			for (Procesoproperty procesopropertyListProcesopropertyToAttach : proceso.getProcesopropertyList()) {
				...
				attachedProcesopropertyList.add(procesopropertyListProcesopropertyToAttach);
			}
			...
			for (Perfilproceso perfilprocesoListPerfilprocesoToAttach : proceso.getPerfilprocesoList()) {
			...
			em.persist(proceso);
			for (Procesoproperty procesopropertyListProcesoproperty : proceso.getProcesopropertyList()) {
				...
				procesopropertyListProcesoproperty = em.merge(procesopropertyListProcesoproperty);
				if (oldIdProcesoOfProcesopropertyListProcesoproperty != null) {
					oldIdProcesoOfProcesopropertyListProcesoproperty.getProcesopropertyList().remove(
			...
			for (Perfilproceso perfilprocesoListPerfilproceso : proceso.getPerfilprocesoList()) {
				Proceso oldIdProcesoOfPerfilprocesoListPerfilproceso = perfilprocesoListPerfilproceso.getIdProceso();
				...
				if (oldIdProcesoOfPerfilprocesoListPerfilproceso != null) {
					oldIdProcesoOfPerfilprocesoListPerfilproceso.getPerfilprocesoList().remove(
	...
	public void edit(Proceso proceso) throws IllegalOrphanException, NonexistentEntityException, Exception {
		...
		try {
			...
			List<Procesoproperty> procesopropertyListOld = persistentProceso.getProcesopropertyList();
			List<Procesoproperty> procesopropertyListNew = proceso.getProcesopropertyList();
			List<Perfilproceso> perfilprocesoListOld = persistentProceso.getPerfilprocesoList();
			List<Perfilproceso> perfilprocesoListNew = proceso.getPerfilprocesoList();
			List<String> illegalOrphanMessages = null;
			for (Procesoproperty procesopropertyListOldProcesoproperty : procesopropertyListOld) {
				if (!procesopropertyListNew.contains(procesopropertyListOldProcesoproperty)) {
					if (illegalOrphanMessages == null) {
						illegalOrphanMessages = new ArrayList<String>();
					}
					illegalOrphanMessages.add("You must retain Procesoproperty "
							+ procesopropertyListOldProcesoproperty + " since its idProceso field is not nullable.");
				}
			}
			...
			for (Procesoproperty procesopropertyListNewProcesopropertyToAttach : procesopropertyListNew) {
				procesopropertyListNewProcesopropertyToAttach = em.getReference(
						procesopropertyListNewProcesopropertyToAttach.getClass(),
						procesopropertyListNewProcesopropertyToAttach.getId());
				attachedProcesopropertyListNew.add(procesopropertyListNewProcesopropertyToAttach);
			}
			...
			for (Procesoproperty procesopropertyListNewProcesoproperty : procesopropertyListNew) {
				if (!procesopropertyListOld.contains(procesopropertyListNewProcesoproperty)) {
					...
					procesopropertyListNewProcesoproperty = em.merge(procesopropertyListNewProcesoproperty);
					if (oldIdProcesoOfProcesopropertyListNewProcesoproperty != null
							&& !oldIdProcesoOfProcesopropertyListNewProcesoproperty.equals(proceso)) {
						oldIdProcesoOfProcesopropertyListNewProcesoproperty.getProcesopropertyList().remove(
			...
			for (Perfilproceso perfilprocesoListNewPerfilproceso : perfilprocesoListNew) {
				if (!perfilprocesoListOld.contains(perfilprocesoListNewPerfilproceso)) {
					...
					if (oldIdProcesoOfPerfilprocesoListNewPerfilproceso != null
							&& !oldIdProcesoOfPerfilprocesoListNewPerfilproceso.equals(proceso)) {
						oldIdProcesoOfPerfilprocesoListNewPerfilproceso.getPerfilprocesoList().remove(
	...
	public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			List<String> illegalOrphanMessages = null;
			List<Procesoproperty> procesopropertyListOrphanCheck = proceso.getProcesopropertyList();
			for (Procesoproperty procesopropertyListOrphanCheckProcesoproperty : procesopropertyListOrphanCheck) {
				if (illegalOrphanMessages == null) {
					illegalOrphanMessages = new ArrayList<String>();
				}
				illegalOrphanMessages.add("This Proceso (" + proceso
			...
			List<Perfilproceso> perfilprocesoListOrphanCheck = proceso.getPerfilprocesoList();
			for (Perfilproceso perfilprocesoListOrphanCheckPerfilproceso : perfilprocesoListOrphanCheck) {
				if (illegalOrphanMessages == null) {
					illegalOrphanMessages = new ArrayList<String>();
				}
				illegalOrphanMessages.add("This Proceso (" + proceso + ") cannot be destroyed since the Perfilproceso "
