...
public class ReportezJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Reportez reportez) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findReportez(reportez.getId()) != null) {
				throw new PreexistingEntityException("Reportez " + reportez + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Reportez reportez) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Reportez persistentReportez = em.find(Reportez.class, reportez.getId());
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findReportez(id) == null) {
					throw new NonexistentEntityException("The reportez with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The reportez with id " + id + " no longer exists.", enfe);
			}
			Sesion idSesion = reportez.getIdSesion();
			if (idSesion != null) {
	 ...
	 * @return List<Reportez>
	 */
	private List<Reportez> findReportezEntities(boolean all, int maxResults, int firstResult) {
