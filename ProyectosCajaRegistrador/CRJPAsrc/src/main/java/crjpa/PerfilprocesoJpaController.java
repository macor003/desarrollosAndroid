...
public class PerfilprocesoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Perfilproceso perfilproceso) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findPerfilproceso(perfilproceso.getId()) != null) {
				throw new PreexistingEntityException("Perfilproceso " + perfilproceso + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Perfilproceso perfilproceso) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Perfilproceso persistentPerfilproceso = em.find(Perfilproceso.class, perfilproceso.getId());
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPerfilproceso(id) == null) {
					throw new NonexistentEntityException("The perfilproceso with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The perfilproceso with id " + id + " no longer exists.", enfe);
			}
			Perfil idPerfil = perfilproceso.getIdPerfil();
			if (idPerfil != null) {
	 ...
	 * @return List<Perfilproceso>
	 */
	public List<Perfilproceso> findPerfilprocesoEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Perfilproceso>
	 */
	private List<Perfilproceso> findPerfilprocesoEntities(boolean all, int maxResults, int firstResult) {
