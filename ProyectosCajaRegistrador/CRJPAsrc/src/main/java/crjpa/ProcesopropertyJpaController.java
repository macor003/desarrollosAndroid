...
public class ProcesopropertyJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Procesoproperty procesoproperty) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findProcesoproperty(procesoproperty.getId()) != null) {
				throw new PreexistingEntityException("Procesoproperty " + procesoproperty + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Procesoproperty procesoproperty) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Procesoproperty persistentProcesoproperty = em.find(Procesoproperty.class, procesoproperty.getId());
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findProcesoproperty(id) == null) {
					throw new NonexistentEntityException("The procesoproperty with id " + id + " no longer exists.");
				}
			}
			throw ex;
		} finally {
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The procesoproperty with id " + id + " no longer exists.", enfe);
			}
			Proceso idProceso = procesoproperty.getIdProceso();
			if (idProceso != null) {
	 ...
	 * @return List<Procesoproperty>
	 */
	public List<Procesoproperty> findProcesopropertyEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Procesoproperty>
	 */
	private List<Procesoproperty> findProcesopropertyEntities(boolean all, int maxResults, int firstResult) {
