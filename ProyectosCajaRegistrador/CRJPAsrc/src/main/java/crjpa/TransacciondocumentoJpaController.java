...
public class TransacciondocumentoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Transacciondocumento transacciondocumento) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (idComprobantefiscalpreimpreso != null) {
				...
				transacciondocumento.setIdComprobantefiscalpreimpreso(idComprobantefiscalpreimpreso);
			}
			...
			Tipodocumento idTipodocumento = transacciondocumento.getIdTipodocumento();
			...
			if (idComprobantefiscalpreimpreso != null) {
				idComprobantefiscalpreimpreso.getTransacciondocumentoList().add(transacciondocumento);
				idComprobantefiscalpreimpreso = em.merge(idComprobantefiscalpreimpreso);
			}
			if (idTransaccion != null) {
				idTransaccion.getTransacciondocumentoList().add(transacciondocumento);
				idTransaccion = em.merge(idTransaccion);
			}
			if (idTipodocumento != null) {
				idTipodocumento.getTransacciondocumentoList().add(transacciondocumento);
				idTipodocumento = em.merge(idTipodocumento);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findTransacciondocumento(transacciondocumento.getId()) != null) {
				throw new PreexistingEntityException("Transacciondocumento " + transacciondocumento
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Transacciondocumento transacciondocumento) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Transacciondocumento persistentTransacciondocumento = em.find(Transacciondocumento.class,
			...
			Comprobantefiscalpreimpreso idComprobantefiscalpreimpresoNew = transacciondocumento
					.getIdComprobantefiscalpreimpreso();
			Transaccion idTransaccionOld = persistentTransacciondocumento.getIdTransaccion();
			Transaccion idTransaccionNew = transacciondocumento.getIdTransaccion();
			Tipodocumento idTipodocumentoOld = persistentTransacciondocumento.getIdTipodocumento();
			Tipodocumento idTipodocumentoNew = transacciondocumento.getIdTipodocumento();
			if (idComprobantefiscalpreimpresoNew != null) {
				...
				transacciondocumento.setIdComprobantefiscalpreimpreso(idComprobantefiscalpreimpresoNew);
			}
			...
			if (idComprobantefiscalpreimpresoOld != null
				...
				idComprobantefiscalpreimpresoOld.getTransacciondocumentoList().remove(transacciondocumento);
			...
			if (idComprobantefiscalpreimpresoNew != null
				...
				idComprobantefiscalpreimpresoNew.getTransacciondocumentoList().add(transacciondocumento);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTransacciondocumento(id) == null) {
					throw new NonexistentEntityException("The transacciondocumento with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				transacciondocumento = em.getReference(Transacciondocumento.class, id);
				transacciondocumento.getId();
			} catch (EntityNotFoundException enfe) {
				throw new NonexistentEntityException("The transacciondocumento with id " + id + " no longer exists.",
						enfe);
			}
			Comprobantefiscalpreimpreso idComprobantefiscalpreimpreso = transacciondocumento
					.getIdComprobantefiscalpreimpreso();
			if (idComprobantefiscalpreimpreso != null) {
				idComprobantefiscalpreimpreso.getTransacciondocumentoList().remove(transacciondocumento);
				idComprobantefiscalpreimpreso = em.merge(idComprobantefiscalpreimpreso);
			}
			Transaccion idTransaccion = transacciondocumento.getIdTransaccion();
			if (idTransaccion != null) {
				idTransaccion.getTransacciondocumentoList().remove(transacciondocumento);
				idTransaccion = em.merge(idTransaccion);
			}
			Tipodocumento idTipodocumento = transacciondocumento.getIdTipodocumento();
			if (idTipodocumento != null) {
				idTipodocumento.getTransacciondocumentoList().remove(transacciondocumento);
				idTipodocumento = em.merge(idTipodocumento);
			}
	 ...
	 * @return List<Transacciondocumento>
	 */
	public List<Transacciondocumento> findTransacciondocumentoEntities(int maxResults, int firstResult) {
	 ...
	 * @return List<Transacciondocumento>
	 */
	private List<Transacciondocumento> findTransacciondocumentoEntities(boolean all, int maxResults, int firstResult) {
