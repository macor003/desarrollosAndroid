...
public class EntregaJpaController implements Serializable {
	...
	public void create(Entrega entrega) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (idUsuarioautorizante != null) {
				idUsuarioautorizante = em.getReference(idUsuarioautorizante.getClass(), idUsuarioautorizante.getId());
			...
			for (Entregaformadepago entregaformadepagoListEntregaformadepago : entrega.getEntregaformadepagoList()) {
				...
				if (oldIdEntregaOfEntregaformadepagoListEntregaformadepago != null) {
					oldIdEntregaOfEntregaformadepagoListEntregaformadepago.getEntregaformadepagoList().remove(
	...
	public void edit(Entrega entrega) throws IllegalOrphanException, NonexistentEntityException, Exception {
		...
		try {
			...
			Usuario idUsuarioautorizanteOld = persistentEntrega.getIdUsuarioautorizante();
			...
			List<Entregaformadepago> entregaformadepagoListOld = persistentEntrega.getEntregaformadepagoList();
			List<Entregaformadepago> entregaformadepagoListNew = entrega.getEntregaformadepagoList();
			...
			if (idUsuarioautorizanteNew != null) {
				idUsuarioautorizanteNew = em.getReference(idUsuarioautorizanteNew.getClass(),
			...
			if (idUsuarioautorizanteOld != null && !idUsuarioautorizanteOld.equals(idUsuarioautorizanteNew)) {
				idUsuarioautorizanteOld.getEntregaList().remove(entrega);
				idUsuarioautorizanteOld = em.merge(idUsuarioautorizanteOld);
			}
			if (idUsuarioautorizanteNew != null && !idUsuarioautorizanteNew.equals(idUsuarioautorizanteOld)) {
			...
			for (Entregaformadepago entregaformadepagoListNewEntregaformadepago : entregaformadepagoListNew) {
				if (!entregaformadepagoListOld.contains(entregaformadepagoListNewEntregaformadepago)) {
					Entrega oldIdEntregaOfEntregaformadepagoListNewEntregaformadepago = entregaformadepagoListNewEntregaformadepago
							.getIdEntrega();
					entregaformadepagoListNewEntregaformadepago.setIdEntrega(entrega);
					...
					if (oldIdEntregaOfEntregaformadepagoListNewEntregaformadepago != null
							&& !oldIdEntregaOfEntregaformadepagoListNewEntregaformadepago.equals(entrega)) {
						oldIdEntregaOfEntregaformadepagoListNewEntregaformadepago.getEntregaformadepagoList().remove(
	...
	public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			List<String> illegalOrphanMessages = null;
			List<Entregaformadepago> entregaformadepagoListOrphanCheck = entrega.getEntregaformadepagoList();
			for (Entregaformadepago entregaformadepagoListOrphanCheckEntregaformadepago : entregaformadepagoListOrphanCheck) {
				if (illegalOrphanMessages == null) {
					illegalOrphanMessages = new ArrayList<String>();
				}
				illegalOrphanMessages.add("This Entrega (" + entrega
