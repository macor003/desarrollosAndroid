...
public class TipoclientetipodocumentoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Tipoclientetipodocumento tipoclientetipodocumento) throws PreexistingEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Tipocliente idTipocliente = tipoclientetipodocumento.getIdTipocliente();
			...
			Tipodocumento idTipodocumento = tipoclientetipodocumento.getIdTipodocumento();
			...
			if (idTipocliente != null) {
				idTipocliente.getTipoclientetipodocumentoList().add(tipoclientetipodocumento);
				idTipocliente = em.merge(idTipocliente);
			}
			if (idTipodocumento != null) {
				idTipodocumento.getTipoclientetipodocumentoList().add(tipoclientetipodocumento);
				idTipodocumento = em.merge(idTipodocumento);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findTipoclientetipodocumento(tipoclientetipodocumento.getId()) != null) {
				throw new PreexistingEntityException("Tipoclientetipodocumento " + tipoclientetipodocumento
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Tipoclientetipodocumento tipoclientetipodocumento) throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Tipoclientetipodocumento persistentTipoclientetipodocumento = em.find(Tipoclientetipodocumento.class,
					tipoclientetipodocumento.getId());
			Tipocliente idTipoclienteOld = persistentTipoclientetipodocumento.getIdTipocliente();
			Tipocliente idTipoclienteNew = tipoclientetipodocumento.getIdTipocliente();
			Tipodocumento idTipodocumentoOld = persistentTipoclientetipodocumento.getIdTipodocumento();
			Tipodocumento idTipodocumentoNew = tipoclientetipodocumento.getIdTipodocumento();
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findTipoclientetipodocumento(id) == null) {
					throw new NonexistentEntityException("The tipoclientetipodocumento with id " + id
							+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				tipoclientetipodocumento = em.getReference(Tipoclientetipodocumento.class, id);
				tipoclientetipodocumento.getId();
			...
			Tipocliente idTipocliente = tipoclientetipodocumento.getIdTipocliente();
			if (idTipocliente != null) {
				idTipocliente.getTipoclientetipodocumentoList().remove(tipoclientetipodocumento);
				idTipocliente = em.merge(idTipocliente);
			}
			Tipodocumento idTipodocumento = tipoclientetipodocumento.getIdTipodocumento();
			if (idTipodocumento != null) {
				idTipodocumento.getTipoclientetipodocumentoList().remove(tipoclientetipodocumento);
				idTipodocumento = em.merge(idTipodocumento);
			}
	 ...
	 * @return List<Tipoclientetipodocumento>
	 */
	public List<Tipoclientetipodocumento> findTipoclientetipodocumentoEntities(int maxResults, int firstResult) {
		return findTipoclientetipodocumentoEntities(false, maxResults, firstResult);
	}
	 ...
	 * @return List<Tipoclientetipodocumento>
	 */
	private List<Tipoclientetipodocumento> findTipoclientetipodocumentoEntities(boolean all, int maxResults,
			int firstResult) {
	...
	public int getTipoclientetipodocumentoCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Tipoclientetipodocumento> rt = cq.from(Tipoclientetipodocumento.class);
