...
public class PerfilJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Perfil perfil) throws PreexistingEntityException,
			Exception {
		...
		try {
			...
			for (Perfilproceso perfilprocesoListPerfilprocesoToAttach : perfil
			...
			for (Usuarioperfil usuarioperfilListUsuarioperfilToAttach : perfil
			...
			em.persist(perfil);
			for (Perfilproceso perfilprocesoListPerfilproceso : perfil
					.getPerfilprocesoList()) {
				Perfil oldIdPerfilOfPerfilprocesoListPerfilproceso = perfilprocesoListPerfilproceso
						.getIdPerfil();
				...
				if (oldIdPerfilOfPerfilprocesoListPerfilproceso != null) {
					oldIdPerfilOfPerfilprocesoListPerfilproceso
							.getPerfilprocesoList().remove(
									perfilprocesoListPerfilproceso);
					oldIdPerfilOfPerfilprocesoListPerfilproceso = em
							.merge(oldIdPerfilOfPerfilprocesoListPerfilproceso);
				}
			}
			for (Usuarioperfil usuarioperfilListUsuarioperfil : perfil
					.getUsuarioperfilList()) {
				Perfil oldIdPerfilOfUsuarioperfilListUsuarioperfil = usuarioperfilListUsuarioperfil
						.getIdPerfil();
				...
				if (oldIdPerfilOfUsuarioperfilListUsuarioperfil != null) {
					oldIdPerfilOfUsuarioperfilListUsuarioperfil
							.getUsuarioperfilList().remove(
									usuarioperfilListUsuarioperfil);
					oldIdPerfilOfUsuarioperfilListUsuarioperfil = em
							.merge(oldIdPerfilOfUsuarioperfilListUsuarioperfil);
				}
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findPerfil(perfil.getId()) != null) {
				throw new PreexistingEntityException("Perfil " + perfil
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	 ...
	 * @throws Exception
	 */
	public void edit(Perfil perfil) throws IllegalOrphanException,
		...
		try {
			...
			List<Perfilproceso> perfilprocesoListOld = persistentPerfil
					.getPerfilprocesoList();
			List<Perfilproceso> perfilprocesoListNew = perfil
					.getPerfilprocesoList();
			List<Usuarioperfil> usuarioperfilListOld = persistentPerfil
					.getUsuarioperfilList();
			List<Usuarioperfil> usuarioperfilListNew = perfil
					.getUsuarioperfilList();
			...
			for (Perfilproceso perfilprocesoListNewPerfilproceso : perfilprocesoListNew) {
				if (!perfilprocesoListOld
					...
					if (oldIdPerfilOfPerfilprocesoListNewPerfilproceso != null
							&& !oldIdPerfilOfPerfilprocesoListNewPerfilproceso
									.equals(perfil)) {
						oldIdPerfilOfPerfilprocesoListNewPerfilproceso
								.getPerfilprocesoList().remove(
			...
			for (Usuarioperfil usuarioperfilListNewUsuarioperfil : usuarioperfilListNew) {
				if (!usuarioperfilListOld
					...
					if (oldIdPerfilOfUsuarioperfilListNewUsuarioperfil != null
							&& !oldIdPerfilOfUsuarioperfilListNewUsuarioperfil
									.equals(perfil)) {
						oldIdPerfilOfUsuarioperfilListNewUsuarioperfil
								.getUsuarioperfilList().remove(
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPerfil(id) == null) {
					throw new NonexistentEntityException("The perfil with id "
							+ id + " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws IllegalOrphanException,
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The perfil with id " + id
						+ " no longer exists.", enfe);
			}
			List<String> illegalOrphanMessages = null;
			List<Perfilproceso> perfilprocesoListOrphanCheck = perfil
					.getPerfilprocesoList();
			...
			List<Usuarioperfil> usuarioperfilListOrphanCheck = perfil
					.getUsuarioperfilList();
	 ...
	 * @return List<Perfil>
	 */
	private List<Perfil> findPerfilEntities(boolean all, int maxResults,
			int firstResult) {
