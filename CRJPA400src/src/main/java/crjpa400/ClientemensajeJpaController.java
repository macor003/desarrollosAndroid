...
public class ClientemensajeJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Clientemensaje clientemensaje)
			throws PreexistingEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Cliente numeroIdentificacionCliente = clientemensaje
			...
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getClientemensajeList().add(
						clientemensaje);
			...
			if (findClientemensaje(clientemensaje.getId()) != null) {
				throw new PreexistingEntityException("Clientemensaje "
						+ clientemensaje + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Clientemensaje clientemensaje)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Clientemensaje persistentClientemensaje = em.find(
					Clientemensaje.class, clientemensaje.getId());
			Cliente numeroIdentificacionClienteOld = persistentClientemensaje
					.getNumeroIdentificacionCliente();
			Cliente numeroIdentificacionClienteNew = clientemensaje
					.getNumeroIdentificacionCliente();
			...
			if (numeroIdentificacionClienteOld != null
				...
				numeroIdentificacionClienteOld.getClientemensajeList().remove(
						clientemensaje);
			...
			if (numeroIdentificacionClienteNew != null
				...
				numeroIdentificacionClienteNew.getClientemensajeList().add(
						clientemensaje);
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			Cliente numeroIdentificacionCliente = clientemensaje
					.getNumeroIdentificacionCliente();
			if (numeroIdentificacionCliente != null) {
				numeroIdentificacionCliente.getClientemensajeList().remove(
						clientemensaje);
				numeroIdentificacionCliente = em
						.merge(numeroIdentificacionCliente);
			}
	 ...
	 * @return List<Clientemensaje>
	 */
	public List<Clientemensaje> findClientemensajeEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Clientemensaje>
	 */
	private List<Clientemensaje> findClientemensajeEntities(boolean all,
