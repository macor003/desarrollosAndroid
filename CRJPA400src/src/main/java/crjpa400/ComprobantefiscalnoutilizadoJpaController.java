...
public class ComprobantefiscalnoutilizadoJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Comprobantefiscalnoutilizado comprobantefiscalnoutilizado)
		...
		try {
			...
			if (idComprobantefiscalpreimpreso != null) {
				...
				comprobantefiscalnoutilizado
						.setIdComprobantefiscalpreimpreso(idComprobantefiscalpreimpreso);
			}
			em.persist(comprobantefiscalnoutilizado);
			if (idComprobantefiscalpreimpreso != null) {
				idComprobantefiscalpreimpreso
						.getComprobantefiscalnoutilizadoList().add(
								comprobantefiscalnoutilizado);
			...
			if (findComprobantefiscalnoutilizado(comprobantefiscalnoutilizado
					.getId()) != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Comprobantefiscalnoutilizado comprobantefiscalnoutilizado)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Comprobantefiscalnoutilizado persistentComprobantefiscalnoutilizado = em
					.find(Comprobantefiscalnoutilizado.class,
			...
			if (idComprobantefiscalpreimpresoNew != null) {
				...
				comprobantefiscalnoutilizado
						.setIdComprobantefiscalpreimpreso(idComprobantefiscalpreimpresoNew);
			}
			comprobantefiscalnoutilizado = em
					.merge(comprobantefiscalnoutilizado);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findComprobantefiscalnoutilizado(id) == null) {
					throw new NonexistentEntityException(
							"The comprobantefiscalnoutilizado with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				comprobantefiscalnoutilizado = em.getReference(
						Comprobantefiscalnoutilizado.class, id);
				comprobantefiscalnoutilizado.getId();
	...
	public List<Comprobantefiscalnoutilizado> findComprobantefiscalnoutilizadoEntities(
		...
		return findComprobantefiscalnoutilizadoEntities(false, maxResults,
				firstResult);
	}
	 ...
	 * @return List<Comprobantefiscalnoutilizado>
	 */
	private List<Comprobantefiscalnoutilizado> findComprobantefiscalnoutilizadoEntities(
	...
	public int getComprobantefiscalnoutilizadoCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Comprobantefiscalnoutilizado> rt = cq
					.from(Comprobantefiscalnoutilizado.class);
