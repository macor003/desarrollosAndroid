...
public class PorcentajeimpuestoretencionJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Porcentajeimpuestoretencion porcentajeimpuestoretencion)
			throws PreexistingEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Formadepago idFormadepago = porcentajeimpuestoretencion
					.getIdFormadepago();
			...
			if (idFormadepago != null) {
				idFormadepago.getPorcentajeimpuestoretencionList().add(
						porcentajeimpuestoretencion);
				idFormadepago = em.merge(idFormadepago);
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			if (findPorcentajeimpuestoretencion(porcentajeimpuestoretencion
					.getId()) != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Porcentajeimpuestoretencion porcentajeimpuestoretencion)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Porcentajeimpuestoretencion persistentPorcentajeimpuestoretencion = em
					.find(Porcentajeimpuestoretencion.class,
							porcentajeimpuestoretencion.getId());
			Formadepago idFormadepagoOld = persistentPorcentajeimpuestoretencion
					.getIdFormadepago();
			Formadepago idFormadepagoNew = porcentajeimpuestoretencion
					.getIdFormadepago();
			...
			if (idFormadepagoOld != null
				...
				idFormadepagoOld.getPorcentajeimpuestoretencionList().remove(
						porcentajeimpuestoretencion);
				idFormadepagoOld = em.merge(idFormadepagoOld);
			}
			if (idFormadepagoNew != null
					&& !idFormadepagoNew.equals(idFormadepagoOld)) {
				idFormadepagoNew.getPorcentajeimpuestoretencionList().add(
						porcentajeimpuestoretencion);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPorcentajeimpuestoretencion(id) == null) {
					throw new NonexistentEntityException(
							"The porcentajeimpuestoretencion with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				porcentajeimpuestoretencion = em.getReference(
						Porcentajeimpuestoretencion.class, id);
				porcentajeimpuestoretencion.getId();
			...
			Formadepago idFormadepago = porcentajeimpuestoretencion
					.getIdFormadepago();
			if (idFormadepago != null) {
				idFormadepago.getPorcentajeimpuestoretencionList().remove(
						porcentajeimpuestoretencion);
				idFormadepago = em.merge(idFormadepago);
			}
	...
	public List<Porcentajeimpuestoretencion> findPorcentajeimpuestoretencionEntities(
		...
		return findPorcentajeimpuestoretencionEntities(false, maxResults,
				firstResult);
	}
	 ...
	 * @return List<Porcentajeimpuestoretencion>
	 */
	private List<Porcentajeimpuestoretencion> findPorcentajeimpuestoretencionEntities(
	...
	public int getPorcentajeimpuestoretencionCount() {
		...
		try {
			CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
			Root<Porcentajeimpuestoretencion> rt = cq
					.from(Porcentajeimpuestoretencion.class);
