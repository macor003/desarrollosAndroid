...
public class MonedatasacambioJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Monedatasacambio monedatasacambio)
		...
		try {
			...
			if (findMonedatasacambio(monedatasacambio.getId()) != null) {
				throw new PreexistingEntityException("Monedatasacambio "
						+ monedatasacambio + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Monedatasacambio monedatasacambio)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Monedatasacambio persistentMonedatasacambio = em.find(
					Monedatasacambio.class, monedatasacambio.getId());
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findMonedatasacambio(id) == null) {
					throw new NonexistentEntityException(
							"The monedatasacambio with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	 ...
	 * @return List<Monedatasacambio>
	 */
	public List<Monedatasacambio> findMonedatasacambioEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Monedatasacambio>
	 */
	private List<Monedatasacambio> findMonedatasacambioEntities(boolean all,
