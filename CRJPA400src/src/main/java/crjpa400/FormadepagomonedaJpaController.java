...
public class FormadepagomonedaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Formadepagomoneda formadepagomoneda)
		...
		try {
			...
			if (findFormadepagomoneda(formadepagomoneda.getId()) != null) {
				throw new PreexistingEntityException("Formadepagomoneda "
						+ formadepagomoneda + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Formadepagomoneda formadepagomoneda)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Formadepagomoneda persistentFormadepagomoneda = em.find(
					Formadepagomoneda.class, formadepagomoneda.getId());
			Moneda idMonedaOld = persistentFormadepagomoneda.getIdMoneda();
			Moneda idMonedaNew = formadepagomoneda.getIdMoneda();
			Formadepago idFormadepagoOld = persistentFormadepagomoneda
					.getIdFormadepago();
			...
			if (idMonedaOld != null && !idMonedaOld.equals(idMonedaNew)) {
				idMonedaOld.getFormadepagomonedaList()
						.remove(formadepagomoneda);
			...
			if (idFormadepagoOld != null
				...
				idFormadepagoOld.getFormadepagomonedaList().remove(
						formadepagomoneda);
				idFormadepagoOld = em.merge(idFormadepagoOld);
			}
			if (idFormadepagoNew != null
					&& !idFormadepagoNew.equals(idFormadepagoOld)) {
				idFormadepagoNew.getFormadepagomonedaList().add(
						formadepagomoneda);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findFormadepagomoneda(id) == null) {
					throw new NonexistentEntityException(
							"The formadepagomoneda with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Long id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				formadepagomoneda = em
						.getReference(Formadepagomoneda.class, id);
			...
			Formadepago idFormadepago = formadepagomoneda.getIdFormadepago();
			if (idFormadepago != null) {
				idFormadepago.getFormadepagomonedaList().remove(
						formadepagomoneda);
				idFormadepago = em.merge(idFormadepago);
			}
	 ...
	 * @return List<Formadepagomoneda>
	 */
	private List<Formadepagomoneda> findFormadepagomonedaEntities(boolean all,
			int maxResults, int firstResult) {
