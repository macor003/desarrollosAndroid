...
public class PromocionfamiliaJpaController implements Serializable {
	 ...
	 * @throws Exception
	 */
	public void create(Promocionfamilia promocionfamilia)
		...
		try {
			...
			if (findPromocionfamilia(promocionfamilia.getId()) != null) {
				throw new PreexistingEntityException("Promocionfamilia "
						+ promocionfamilia + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	 ...
	 * @throws Exception
	 */
	public void edit(Promocionfamilia promocionfamilia)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Promocionfamilia persistentPromocionfamilia = em.find(
					Promocionfamilia.class, promocionfamilia.getId());
			Promocion idPromocionOld = persistentPromocionfamilia
					.getIdPromocion();
			...
			if (idPromocionOld != null
				...
				idPromocionOld.getPromocionfamiliaList().remove(
						promocionfamilia);
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findPromocionfamilia(id) == null) {
					throw new NonexistentEntityException(
							"The promocionfamilia with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	 ...
	 * @return List<Promocionfamilia>
	 */
	public List<Promocionfamilia> findPromocionfamiliaEntities(int maxResults,
			int firstResult) {
	 ...
	 * @return List<Promocionfamilia>
	 */
	private List<Promocionfamilia> findPromocionfamiliaEntities(boolean all,
